...
TRUSTED_PROXIES = (
    config("TRUSTED_PROXIES", default="").split(",")
    if config("TRUSTED_PROXIES", default="")
    else []
)
...
if not DEBUG:
    ...
    _insecure_origins = [
        ...
        if not o.strip().startswith("https://") and not _is_local_origin(o)
    ]
    if _insecure_origins:
        logging.warning(
            f"⚠️  Insecure CORS origins configured (should use HTTPS): {_insecure_origins}"
        )
else:
...
DEFAULT_FROM_EMAIL = config(
    "DEFAULT_FROM_EMAIL", default="noreply@versatexanalytics.com"
)
...
AI_INSIGHTS_CACHE_TTL = config(
    "AI_INSIGHTS_CACHE_TTL", default=3600, cast=int
)  # 1 hour
...
Data is scoped by organization. Superusers can specify `organization_id` query parameter.
    ...
    "TAGS": [
        ...
        {
            "name": "Procurement",
            "description": "Suppliers, categories, and transactions",
        },
        {"name": "Analytics", "description": "Spend analytics and insights"},
        {"name": "Reports", "description": "Report generation and scheduling"},
        {
            "name": "P2P Analytics - Cycle Time",
            "description": "P2P process cycle time analysis",
        },
        {
            "name": "P2P Analytics - 3-Way Matching",
            "description": "Invoice matching and exception management",
        },
        {
            "name": "P2P Analytics - Invoice Aging",
            "description": "Accounts payable aging analysis",
        },
        {
            "name": "P2P Analytics - Requisitions",
            "description": "Purchase requisition analytics",
        },
        {
            "name": "P2P Analytics - Purchase Orders",
            "description": "Purchase order analytics and compliance",
        },
        {
            "name": "P2P Analytics - Supplier Payments",
            "description": "Supplier payment performance (Admin only)",
        },
    ],
    "CONTACT": {
        "name": "Defoxx Analytics",
        "url": "https://github.com/DefoxxAnalytics",
    },
...
if not DEBUG:
    ...
    SECURE_HSTS_INCLUDE_SUBDOMAINS = config(
        "HSTS_INCLUDE_SUBDOMAINS", default=False, cast=bool
    )
...
_logging_handlers = {
    "console": {
        ...
        "formatter": "verbose",
        # On Railway/Docker, stdout streams to a searchable log collector —
        # same redaction the security log file gets must apply here too,
        # otherwise JWTs/api-keys/passwords leak into the platform log index.
...
if not DEBUG:
    ...
    if (
        "MUST_CHANGE" in _db_password
        or "CHANGE" in _db_password
        or _db_password == "analytics_pass"
    ):
        raise ImproperlyConfigured(
    ...
    if (
        not _redis_password
        or "MUST_CHANGE" in _redis_password
        or "CHANGE" in _redis_password
    ):
        raise ImproperlyConfigured(
