...
class SupplierFactory(DjangoModelFactory):
    ...
    contact_email = factory.LazyAttribute(
        lambda obj: f'{obj.name.replace(" ", ".")}@example.com'
    )
...
class TransactionFactory(DjangoModelFactory):
    ...
    supplier = factory.SubFactory(
        SupplierFactory, organization=factory.SelfAttribute("..organization")
    )
    category = factory.SubFactory(
        CategoryFactory, organization=factory.SelfAttribute("..organization")
    )
    amount = factory.LazyFunction(
        lambda: Decimal(str(round(1000 + 9000 * random.random(), 2)))
    )
...
class DataUploadFactory(DjangoModelFactory):
    ...
    failed_rows = factory.LazyAttribute(
        lambda obj: obj.total_rows - obj.successful_rows - obj.duplicate_rows
    )
...
class LargeTransactionFactory(TransactionFactory):
    ...
    amount = factory.LazyFunction(
        lambda: Decimal(str(round(100000 + 900000 * random.random(), 2)))
    )
    spend_band = ">100K"
...
class SmallTransactionFactory(TransactionFactory):
    ...
    amount = factory.LazyFunction(
        lambda: Decimal(str(round(10 + 990 * random.random(), 2)))
    )
    spend_band = "<1K"
