...
class TestSupplierViewSet:
    ...
    def test_list_suppliers_organization_scoped(
    ...
    def test_create_supplier_creates_audit_log(self, admin_client, admin_user):
        ...
        log = AuditLog.objects.filter(
            user=admin_user, action="create", resource="supplier"
        ).first()
    ...
    def test_cannot_access_other_org_supplier(
        self, authenticated_client, other_organization
    ):
...
class TestCategoryViewSet:
    ...
    def test_create_category(self, admin_client, organization):
        ...
        url = reverse("category-list")
        data = {"name": "New Category", "description": "A new category for testing"}
        response = admin_client.post(url, data)
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_create_category_with_parent(self, admin_client, category):
        ...
        url = reverse("category-list")
        data = {"name": "Child Category", "parent": category.id}
        response = admin_client.post(url, data)
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data["parent"] == category.id

    def test_category_organization_scoped(
        self, authenticated_client, other_organization
    ):
...
class TestTransactionViewSet:
    ...
    def test_transaction_organization_scoped(
    ...
    def test_filter_transactions_by_supplier(
        self, authenticated_client, organization, supplier, category, admin_user
    ):
...
Test,Test,100.00,2024-01-01"""
        ...
        response = authenticated_client.post(
            url, {"file": csv_file}, format="multipart"
        )
...
Test Supplier,Test Category,500.00,2024-03-01"""
        ...
        log = AuditLog.objects.filter(
            user=manager_user, action="upload", resource="transactions"
        ).first()
        assert log is not None
...
class TestBulkDelete:
    ...
    def test_bulk_delete_success(
        self, admin_client, organization, supplier, category, admin_user
    ):
    ...
    def test_bulk_delete_viewer_forbidden(self, authenticated_client, transaction):
        ...
        url = reverse("transaction-bulk-delete")
        response = authenticated_client.post(
            url, {"ids": [transaction.id]}, format="json"
        )
        assert response.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_bulk_delete_creates_audit_log(
        self, admin_client, admin_user, organization, supplier, category
    ):
        ...
        log = AuditLog.objects.filter(
            user=admin_user, action="delete", resource="transactions"
        ).first()
        assert log is not None
...
class TestExport:
    ...
    def test_export_with_date_filters(
        ...
        response = authenticated_client.get(
            url, {"start_date": "2024-01-01", "end_date": "2024-03-31"}
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_export_creates_audit_log(self, authenticated_client, user, transaction):
        ...
        log = AuditLog.objects.filter(
            user=user, action="export", resource="transactions"
        ).first()
        assert log is not None
...
class TestDataUploadViewSet:
    ...
    def test_uploads_organization_scoped(
        self, authenticated_client, other_organization, other_org_user
    ):
        """Test that uploads from other orgs are not visible."""
        other_upload = DataUploadFactory(
            organization=other_organization, uploaded_by=other_org_user
        )
    ...
    def test_uploads_readonly(self, admin_client, organization, admin_user):
        ...
        url = reverse("upload-list")
        data = {"file_name": "test.csv", "file_size": 1024, "batch_id": "test-batch"}
        response = admin_client.post(url, data)
...
class TestOrganizationIsolation:
    ...
    def test_cannot_delete_other_org_transaction(
        self, admin_client, other_organization, other_org_user
    ):
    ...
    def test_bulk_delete_ignores_other_org_transactions(
        self,
        admin_client,
        organization,
        other_organization,
        ...
        response = admin_client.post(
            url, {"ids": [own_tx.id, other_tx.id]}, format="json"
        )
        assert response.status_code == status.HTTP_200_OK
