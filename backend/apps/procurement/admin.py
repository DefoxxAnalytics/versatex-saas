...
from .forms import CSVUploadForm, DeleteAllDataForm, OrganizationResetForm
from .models import (
    Category,
...
from .services import CSVProcessor, get_or_create_category, get_or_create_supplier
...
def _record_import_failure(stats, row_num, exc, doc_type):
    ...
    logger.exception("P2P %s CSV import row %s failed", doc_type, row_num)
...
class DataUploadAdmin(admin.ModelAdmin):
    ...
    def upload_csv_view(self, request):
        ...
        if not request.user.is_superuser:
            if not hasattr(request.user, "profile"):
                messages.error(
                    request, "You do not have a user profile. Contact an administrator."
                )
                return redirect("..")
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                try:
                    ...
                    if not organization:
                        messages.error(
                            request, "Could not determine target organization."
                        )
                        return redirect(".")
                    ...
                    if is_super_admin and len(processor.orgs_affected) > 0:
                        audit_details["organizations_affected"] = list(
                            processor.orgs_affected
                        )
    ...
    def reset_organization_view(self, request):
        ...
        if not request.user.is_superuser:
            messages.error(
                request, "Only super administrators can reset organizations."
            )
            return redirect("..")
                ...
                with transaction.atomic():
                    ...
                    log_action(
                        ...
                        details={
                            ...
                            "purchase_requisitions_deleted": counts[
        ...
        return render(
            request, "admin/procurement/dataupload/reset_organization.html", context
        )
    ...
    def delete_all_data_view(self, request):
        ...
        if not request.user.is_superuser:
            ...
            if request.user.profile.role != "admin":
                messages.error(
                    request, "Only organization administrators can delete data."
                )
                return redirect("..")
        ...
        return render(
            request, "admin/procurement/dataupload/delete_all_data.html", context
        )
    ...
    def upload_wizard_view(self, request):
        ...
        return render(
            request, "admin/procurement/dataupload/upload_wizard.html", context
        )
    ...
    def api_preview_file(self, request):
        ...
        if file.size > 50 * 1024 * 1024:
            return JsonResponse(
                {"error": "File size must be less than 50MB"}, status=400
            )
        ...
        try:
            ...
            file.seek(0)
            total_rows = (
                sum(1 for _ in csv.reader(io.StringIO(content))) - 1
            )  # Exclude header
            ...
            return JsonResponse(
                {"error": "File encoding not supported. Please use UTF-8."}, status=400
            )
        except Exception as e:
    ...
    def api_validate_mapping(self, request):
        ...
        strict_duplicates = (
            request.POST.get("strict_duplicates", "false").lower() == "true"
        )
        ...
        try:
            ...
            for row_num, row in enumerate(
                ...
                if row_errors:
                    ...
                    if not skip_duplicates and self._is_duplicate_row(
                        row, mapping, organization, strict_mode=strict_duplicates
                    ):
                        duplicate_count += 1
                    else:
                        valid_count += 1
        ...
        try:
            ...
            if use_async:
                ...
                task = process_csv_upload.delay(
                    upload.id, mapping, skip_invalid, skip_duplicates, strict_duplicates
                )
    ...
    def api_list_templates(self, request):
        ...
        organization = self._get_target_organization(
            request, request.GET.get("organization_id")
        )
        ...
        templates = (
            ColumnMappingTemplate.objects.filter(organization=organization)
        ...
        return JsonResponse(
            {"success": True, "templates": list(templates)}, encoder=DjangoJSONEncoder
        )
    ...
    def api_save_template(self, request):
        ...
        try:
            ...
            return JsonResponse(
                {"error": f"Error saving template: {str(e)}"}, status=500
            )
    ...
    def upload_detail_view(self, request, uuid):
        ...
        return render(
            request, "admin/procurement/dataupload/upload_detail.html", context
        )
    ...
    def download_error_report(self, request, uuid):
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="errors_{upload.file_name}"'
        )
    ...
    def _parse_date(self, date_str):
        ...
        v3.1 Phase 2 (P-M1): see twin docstring on tasks._parse_date.
        Ambiguous formats removed so European inputs no longer parse
        silently as US dates.
        """
        formats = [
            "%Y-%m-%d",
            "%Y/%m/%d",
            ...
            for row_num, row in enumerate(reader, start=2):
                try:
                    ...
                    Transaction.objects.create(
                        ...
                        invoice_number=(
                            row.get(invoice_col, "").strip() if invoice_col else ""
                        ),
                        fiscal_year=(
                        ...
                        location=(
                            row.get(location_col, "").strip() if location_col else ""
                        ),
                        spend_band=(
...
class P2PImportMixin:
    ...
    def import_csv_view(self, request):
        ...
        if request.method == "POST":
            ...
            try:
                ...
                batch_id = str(uuid_lib.uuid4())
                stats = self._process_p2p_import(
                    rows, organization, batch_id, request.user
                )
                ...
                if stats["failed"] == 0:
                    ...
                    if error_entries:
                        ...
                        messages.error(request, f"Row failures: {summary}{more}")

                return redirect("..")

            except UnicodeDecodeError:
                messages.error(
                    request, "File encoding not supported. Please use UTF-8."
                )
            except Exception as e:
    ...
    def download_template_view(self, request):
        ...
        response = HttpResponse(content_type="text/csv")
        response["Content-Disposition"] = (
            f'attachment; filename="{self.p2p_doc_type}_template.csv"'
        )
...
class PurchaseRequisitionAdmin(P2PImportMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identity", {"fields": ("organization", "pr_number", "uuid")}),
        (
            "Request Details",
            {"fields": ("requested_by", "department", "cost_center", "description")},
        ),
        ("Content", {"fields": ("supplier_suggested", "category")}),
        ("Financial", {"fields": ("estimated_amount", "currency", "budget_code")}),
        ("Status & Workflow", {"fields": ("status", "priority")}),
        (
            "Key Dates",
            {
                "fields": (
...
class PurchaseOrderAdmin(P2PImportMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identity", {"fields": ("organization", "po_number", "uuid")}),
        ("Supplier & Category", {"fields": ("supplier", "category")}),
        (
            "Financial",
            {"fields": ("total_amount", "currency", "tax_amount", "freight_amount")},
        ),
        ("Contract Linkage", {"fields": ("contract", "is_contract_backed")}),
        ("Status & Workflow", {"fields": ("status",)}),
        (
            "Key Dates",
            {
                "fields": (
        ...
        ("Approvals", {"fields": ("created_by", "approved_by")}),
        ("Amendment Tracking", {"fields": ("original_amount", "amendment_count")}),
        ("PR Linkage", {"fields": ("requisition",)}),
        (
            "Metadata",
            {
                "fields": ("upload_batch", "created_at", "updated_at"),
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if not request.user.is_superuser and hasattr(request.user, "profile"):
            ...
            if db_field.name == "supplier":
                ...
                kwargs["queryset"] = PurchaseRequisition.objects.filter(
            ...
            try:
                ...
                with transaction.atomic():
                    ...
                    PurchaseOrder.objects.create(
                        ...
                        currency=row.get("currency", "USD").strip() or "USD",
                        tax_amount=self._parse_decimal(row.get("tax_amount", ""))
                        or Decimal("0"),
                        freight_amount=self._parse_decimal(
...
class GoodsReceiptAdmin(P2PImportMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ("Identity", {"fields": ("organization", "gr_number", "uuid")}),
        ("PO Linkage", {"fields": ("purchase_order",)}),
        ("Receipt Details", {"fields": ("received_date", "received_by")}),
        (
            "Quantities",
            {
                "fields": (
    ...
    def _process_p2p_import(self, rows, organization, batch_id, user):
        ...
        for row_num, row in enumerate(rows, start=2):
            try:
                ...
                quantity_received = self._parse_decimal(
                ...
                with transaction.atomic():
                    GoodsReceipt.objects.create(
                        ...
                        quantity_ordered=self._parse_decimal(
                            row.get("quantity_ordered", "")
                        )
                        or quantity_received,
                        quantity_received=quantity_received,
                        quantity_accepted=self._parse_decimal(
                            row.get("quantity_accepted", "")
                        )
                        or quantity_received,
                        amount_received=self._parse_decimal(
    ...
    def _get_template_example_row(self):
        return [
            ...
            "24500.00",
            "received",
            "Minor packaging damage, items OK",
        ]
...
class InvoiceAdmin(P2PImportMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Linkage (3-Way Match)", {"fields": ("purchase_order", "goods_receipt")}),
        (
            "Financial",
        ...
        ("Status & Matching", {"fields": ("status", "match_status")}),
        (
            "Exception Tracking",
            {
                "fields": (
        ...
        ("Hold", {"fields": ("hold_reason",), "classes": ("collapse",)}),
        (
            "Metadata",
            {
                "fields": ("upload_batch", "created_at", "updated_at"),
    ...
    def exception_badge(self, obj):
        ...
        if obj.exception_resolved:
            ...
            text = (
                obj.get_exception_type_display() if obj.exception_type else "Exception"
            )
        ...
        for row_num, row in enumerate(rows, start=2):
            try:
                ...
                with transaction.atomic():
                    ...
                    # v3.1 Phase 1 (P-C2): duplicate check must include
                    # `supplier` to match the model's three-column
                    # unique_together (organization, invoice_number, supplier).
                    # The previous (org, invoice_number) filter silently
                    # skipped legitimate cross-supplier same-invoice-number
                    # rows — common in healthcare/manufacturing real data —
                    # and corrupted AP aging + 3-way matching counts.
                    ...
                    has_exception = row.get("has_exception", "").strip().lower() in (
                        "true",
                        "yes",
                        "1",
                    )
                    ...
                    Invoice.objects.create(
                        ...
                        tax_amount=self._parse_decimal(row.get("tax_amount", ""))
                        or Decimal("0"),
                        net_amount=self._parse_decimal(row.get("net_amount", ""))
                        or invoice_amount,
                        payment_terms=row.get("payment_terms", "").strip(),
                        payment_terms_days=int(
                            row.get("payment_terms_days", "30") or "30"
                        ),
                        status=row.get("status", "received").strip() or "received",
                        match_status=row.get("match_status", "unmatched").strip()
                        or "unmatched",
                        purchase_order=purchase_order,
                        ...
                        exception_amount=(
                            self._parse_decimal(row.get("exception_amount", ""))
                            if has_exception
                            else None
                        ),
                        exception_notes=(
                            row.get("exception_notes", "").strip()
                            if has_exception
                            else ""
                        ),
                        upload_batch=batch_id,
