...
try:
    ...
    HAS_MAGIC = False
    logging.warning(
        "python-magic not installed. File type validation will use fallback method."
    )
    ...
    def validate(self, attrs):
        ...
        if not attrs.get("supplier") and not attrs.get("supplier_name"):
            raise serializers.ValidationError(
                {"supplier": "Either supplier ID or supplier_name is required."}
            )
        if not attrs.get("category") and not attrs.get("category_name"):
            raise serializers.ValidationError(
                {"category": "Either category ID or category_name is required."}
            )
        return attrs
...
class CSVUploadSerializer(serializers.Serializer):
    ...
    def validate_file(self, value):
        ...
        if content_type not in self.ALLOWED_CONTENT_TYPES:
            ...
            logger.debug(
                f"Unexpected content type '{content_type}', verifying with magic"
            )
        ...
        try:
            ...
            raise serializers.ValidationError(
                "Error validating file content: Unable to read file"
            )
...
class PurchaseRequisitionSerializer(serializers.ModelSerializer):
    ...
    supplier_suggested_name = serializers.CharField(
        source="supplier_suggested.name", read_only=True
    )
    category_name = serializers.CharField(source="category.name", read_only=True)
