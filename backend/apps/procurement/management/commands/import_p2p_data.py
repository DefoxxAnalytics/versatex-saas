...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--file", type=str, required=True, help="Path to the CSV file to import"
        )
        parser.add_argument(
            "--skip-errors",
            action="store_true",
        ...
        parser.add_argument(
            "--batch-id", type=str, help="Custom batch ID for tracking this import"
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"Importing {doc_type.upper()} data for organization: {organization.name}"
        )
    ...
    def _import_purchase_requisitions(
        self, rows, organization, batch_id, skip_errors, dry_run
    ):
        """
            ...
            try:
                ...
                if not pr_number:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Missing pr_number"}
                        )
                        continue
                ...
                if estimated_amount is None:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid estimated_amount"}
                        )
                        continue
                ...
                if not dry_run:
                    with transaction.atomic():
                        ...
                        PurchaseRequisition.objects.create(
                            ...
                            submitted_date=self._parse_date(
            ...
            try:
                ...
                if not po_number:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Missing po_number"}
                        )
                        continue
                ...
                if not supplier_name:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Missing supplier_name"}
                        )
                        continue
                ...
                if total_amount is None:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid total_amount"}
                        )
                        continue
                ...
                if not dry_run:
                    with transaction.atomic():
                        ...
                        PurchaseOrder.objects.create(
                            ...
                            required_date=self._parse_date(
                                row.get("required_date", "")
                            ),
                            promised_date=self._parse_date(
                                row.get("promised_date", "")
                            ),
            ...
            try:
                ...
                if not gr_number:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Missing gr_number"}
                        )
                        continue
                ...
                if not purchase_order:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": f"PO not found: {po_number}"}
                        )
                        continue
                ...
                if not received_date:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid received_date"}
                        )
                        continue
                ...
                if quantity_received is None:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid quantity_received"}
                        )
                        continue
            ...
            try:
                ...
                if not invoice_number:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Missing invoice_number"}
                        )
                        continue
                ...
                if invoice_amount is None:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid invoice_amount"}
                        )
                        continue
                ...
                if not invoice_date:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid invoice_date"}
                        )
                        continue
                ...
                if not due_date:
                    if skip_errors:
                        stats["failed"] += 1
                        stats["errors"].append(
                            {"row": row_num, "message": "Invalid due_date"}
                        )
                        continue
                ...
                if not dry_run:
                    with transaction.atomic():
                        ...
                        Invoice.objects.create(
                            ...
                            received_date=self._parse_date(row.get("received_date", ""))
                            or invoice_date,
                            approved_date=self._parse_date(
