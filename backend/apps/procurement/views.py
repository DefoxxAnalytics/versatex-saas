...
from apps.authentication.permissions import CanDeleteData, CanUploadData
...
class TransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = Transaction.objects.filter(organization=organization).select_related(
            "supplier", "category", "uploaded_by"
        )
    ...
    def upload_csv(self, request):
        ...
        try:
            ...
            return Response(
                DataUploadSerializer(upload).data, status=status.HTTP_201_CREATED
            )
    ...
    @action(
        ...
        throttle_classes=[BulkDeleteThrottle],
    )
    def bulk_delete(self, request):
        ...
        return Response(
            {"message": f"{count} transactions deleted successfully", "count": count}
        )
    ...
    def export(self, request):
        ...
        df = export_transactions_to_csv(organization=organization, filters=filters)
        ...
        log_action(
            ...
            resource="transactions",
            details=(
                {"count": len(df), "organization_id": organization.id}
