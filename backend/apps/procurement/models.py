...
class Transaction(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="uploaded_transactions"
    )
...
class ColumnMappingTemplate(models.Model):
    ...
    uuid = models.UUIDField(default=uuid.uuid4, editable=False, unique=True)
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="mapping_templates"
    )
...
class DataUpload(models.Model):
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="data_uploads"
    )
    ...
    stored_file = models.FileField(upload_to="uploads/%Y/%m/%d/", null=True, blank=True)
    ...
    processing_mode = models.CharField(
        max_length=10, choices=PROCESSING_MODE_CHOICES, default="sync"
    )
...
class Contract(models.Model):
    ...
    def days_to_expiry(self):
        ...
        v3.1 Phase 2 (P-M4): timezone.now().date() instead of naive
        date.today(). On a UTC-deployed server `date.today()` is UTC and
        flips a day before/after a user's local midnight, off-by-one
        days_to_expiry on the boundary day.
        """
        if self.end_date:
...
class SpendingPolicy(models.Model):
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="spending_policies"
    )
...
class PolicyViolation(models.Model):
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="policy_violations"
    )
    transaction = models.ForeignKey(
        Transaction, on_delete=models.CASCADE, related_name="violations"
    )
    policy = models.ForeignKey(
        SpendingPolicy, on_delete=models.CASCADE, related_name="violations"
    )
...
class PurchaseRequisition(models.Model):
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="purchase_requisitions"
    )
    pr_number = models.CharField(max_length=50)

    # Request details
    requested_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="requisitions_created"
    )
    ...
    category = models.ForeignKey(
        Category, on_delete=models.SET_NULL, null=True, related_name="requisitions"
    )
    description = models.TextField(blank=True)
    ...
    def is_overdue(self):
        ...
        v3.1 Phase 2 (P-M4): timezone-aware day boundary, see Contract
        equivalent above for rationale.
        """
        if self.status == "pending_approval" and self.submitted_date:
            return (timezone.now().date() - self.submitted_date).days > 5
        return False
...
class PurchaseOrder(models.Model):
    ...
    def amount_variance(self):
        ...
        if self.original_amount and self.original_amount > 0:
            return float(
                (self.total_amount - self.original_amount) / self.original_amount * 100
            )
        return 0
...
class GoodsReceipt(models.Model):
    ...
    # Identity - organization scoped for multi-tenancy
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="goods_receipts"
    )
...
class Invoice(models.Model):
    ...
    exception_type = models.CharField(
        max_length=20, choices=EXCEPTION_TYPE_CHOICES, blank=True
    )
    exception_amount = models.DecimalField(
        max_digits=15, decimal_places=2, null=True, blank=True
    )
    ...
    def __str__(self):
        return (
            f"{self.invoice_number} - {self.supplier.name} ({self.organization.name})"
        )
    ...
    def days_outstanding(self):
        ...
        v3.1 Phase 2 (P-M4): timezone.now().date() — see model header.
        """
        if self.paid_date:
    ...
    def is_overdue(self):
        ...
        return (
            self.status not in ["paid", "disputed"]
    ...
    def discount_available(self):
        ...
        if self.discount_days and self.discount_percent and self.invoice_date:
            discount_deadline = self.invoice_date + timedelta(days=self.discount_days)
            return timezone.now().date() <= discount_deadline
        return False
