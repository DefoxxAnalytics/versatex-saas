...
class TestHealthEndpoint:
    ...
    def test_returns_503_when_both_fail(self):
        with (
            patch("apps.health.views.connection") as mock_conn,
    ...
    def test_sets_no_cache_headers(self):
        response = Client().get("/api/health/")
        assert (
            "no-cache" in response.headers.get("Cache-Control", "").lower()
            or "no-store" in response.headers.get("Cache-Control", "").lower()
            or "max-age=0" in response.headers.get("Cache-Control", "").lower()
        )
