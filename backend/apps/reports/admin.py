...
class ReportAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Organization & Ownership", {"fields": ("organization", "created_by")}),
        ("Date Range", {"fields": ("period_start", "period_end")}),
        (
            "Filters & Parameters",
            {"fields": ("filters", "parameters"), "classes": ("collapse",)},
        ),
        ("Status", {"fields": ("status", "error_message", "file_path", "file_size")}),
        (
            "Generated Data",
            {"fields": ("summary_data_preview",), "classes": ("collapse",)},
        ),
        ("Sharing", {"fields": ("is_public", "shared_with")}),
        (
            "Scheduling",
    ...
    def summary_data_preview(self, obj):
        ...
        if len(json.dumps(obj.summary_data)) > 2000:
            preview += "\n... (truncated)"
        return format_html(
            '<pre style="max-height: 400px; overflow: auto;">{}</pre>', preview
        )
    ...
    def get_queryset(self, request):
        ...
        v3.1 Phase 1 (X-2): scopes to all orgs the admin is a member of via
        ``UserOrganizationMembership``, not just their legacy primary
        ``profile.organization``. A multi-org admin viewing the Reports
        admin page previously saw only their primary org's reports, hiding
        reports they had legitimate management rights for in other orgs.
        """
        from apps.authentication.organization_utils import get_user_organizations

        qs = super().get_queryset(request).select_related("organization", "created_by")
        if request.user.is_superuser:
            return qs
        if hasattr(request.user, "profile"):
            return qs.filter(organization__in=get_user_organizations(request.user))
        return qs.none()
