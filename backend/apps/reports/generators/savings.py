...
class SavingsOpportunitiesGenerator(BaseReportGenerator):
    ...
    def generate(self) -> dict:
        ...
        for s in suppliers:
            ...
            if (
                s["count"] >= 3 and s["amount"] < total_spend * 0.01
            ):  # Small supplier, multiple transactions
                estimated_savings = (
                    s["amount"] * 0.15
                )  # Assume 15% savings through consolidation
        ...
        consolidation_opportunities.sort(
        ...
        tail_savings = (
            tail_spend.get("tail_spend", 0) * 0.12
        )  # 12% savings on tail spend

        # Calculate total potential savings
        total_consolidation_savings = sum(
            c["estimated_savings"] for c in consolidation_opportunities[:20]
        )
        total_category_savings = sum(
            c["estimated_savings"] for c in category_opportunities
        )
        ...
        savings_by_type = [
            {
                ...
                "target_spend": sum(
                    c["current_spend"] for c in consolidation_opportunities[:20]
                ),
        ...
        total_potential_savings = (
            total_consolidation_savings + tail_savings + total_category_savings
        )
        ...
        return {
            ...
            "savings_summary": {
                ...
                "savings_percentage": round(
                    (
                        (total_potential_savings / total_spend * 100)
                        if total_spend > 0
                        else 0
                    ),
                    2,
            ...
            "action_plan": [
                ...
                },
                {
                    ...
                    "estimated_savings": sum(
                        c["estimated_savings"] for c in consolidation_opportunities[:10]
                    ),
                ...
                },
                {
                    ...
                    "estimated_savings": (
                        category_opportunities[0]["estimated_savings"]
