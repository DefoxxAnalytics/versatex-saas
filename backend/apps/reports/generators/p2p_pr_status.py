...
class PRStatusReportGenerator(BaseReportGenerator):
    ...
    def generate(self) -> dict:
        ...
        # Generate insights
        insights = self._generate_insights(summary, pr_by_department, approval_analysis)
    ...
    def _generate_insights(self, summary, departments, approval_analysis):
        ...
        if departments:
            approval_rates = [
                d.get("approval_rate", 0)
                for d in departments
                if d.get("pr_count", 0) >= 5
            ]
            if approval_rates:
    ...
    def _generate_recommendations(
        self, summary, departments, pending, approval_analysis
    ):
        ...
        low_approval_depts = [
            d
            for d in departments
            if d.get("approval_rate", 100) < 75 and d.get("pr_count", 0) >= 3
        ]
        if low_approval_depts:
            dept_names = ", ".join(
                [d.get("department", "Unknown") for d in low_approval_depts[:3]]
            )
            recommendations.append(
                {
                    "type": "info",
        ...
        high_value_pending = [
            p for p in pending if p.get("estimated_amount", 0) > 50000
        ]
        if high_value_pending:
            total_pending_value = sum(
                p.get("estimated_amount", 0) for p in high_value_pending
            )
            recommendations.append(
                {
                    "type": "warning",
