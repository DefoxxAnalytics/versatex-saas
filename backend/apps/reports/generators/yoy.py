...
class YearOverYearReportGenerator(BaseReportGenerator):
    ...
    def generate(self) -> dict:
        ...
        yoy_data = self.analytics.get_detailed_year_over_year(
            year1=year1, year2=year2, use_fiscal_year=use_fiscal
        )
        ...
        variance_analysis = self._analyze_variance(category_comparison, summary)
    ...
    def _analyze_variance(self, category_comparison, summary):
        ...
        if categories_increased:
            largest_increase = max(
                categories_increased, key=lambda x: x.get("change", 0)
            )
        if categories_decreased:
            largest_decrease = min(
                categories_decreased, key=lambda x: x.get("change", 0)
            )
        ...
        comparable = [
            ...
            if c.get("year1_spend", 0) > 0 and c.get("year2_spend", 0) > 0
        ]
        avg_change = (
            sum(c.get("change_pct", 0) for c in comparable) / len(comparable)
            if comparable
            else 0
        )
        ...
        return {
            ...
            "discontinued_categories": [
                c.get("category", "") for c in categories_discontinued[:5]
            ],
        }

    def _generate_insights(
        self, summary, top_gainers, top_decliners, variance_analysis
    ):
    ...
    def _generate_recommendations(
        self, summary, top_gainers, top_decliners, category_comparison
    ):
        ...
        if top_gainers:
            ...
            if high_growth_cats:
                cat_names = ", ".join(
                    [c.get("category", "") for c in high_growth_cats[:3]]
                )
        ...
        if top_decliners:
            high_decline_cats = [
                d for d in top_decliners if d.get("change_pct", 0) < -25
            ]
            if high_decline_cats:
                cat_names = ", ".join(
                    [c.get("category", "") for c in high_decline_cats[:3]]
                )
                recommendations.append(
                    {
                        "type": "info",
