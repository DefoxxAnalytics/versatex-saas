...
class POComplianceReportGenerator(BaseReportGenerator):
    ...
    def _generate_insights(self, summary, leakage, amendments, suppliers):
        ...
        maverick_suppliers = [
            s for s in suppliers if s.get("contract_status") == "maverick"
        ]
        if len(maverick_suppliers) > 5:
            total_maverick_value = sum(
                s.get("total_value", 0) for s in maverick_suppliers
            )
            insights.append(
                {
                    "type": "info",
    ...
    def _generate_recommendations(self, summary, leakage, amendments, suppliers):
        ...
        if leakage:
            high_maverick_cats = [
                c for c in leakage if c.get("maverick_percent", 0) > 30
            ]
            if high_maverick_cats:
                cat_names = ", ".join(
                    [c.get("category", "") for c in high_maverick_cats[:3]]
                )
                total_maverick = sum(
                    c.get("maverick_amount", 0) for c in high_maverick_cats
                )
                recommendations.append(
                    {
                        "type": "opportunity",
        ...
        late_suppliers = [
            s
            for s in suppliers
            if s.get("on_time_rate", 100) < 80
            and s.get("contract_status") == "on_contract"
        ]
        if late_suppliers:
            recommendations.append(
                {
