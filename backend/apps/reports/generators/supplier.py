...
class SupplierPerformanceGenerator(BaseReportGenerator):
    ...
    def generate(self) -> dict:
        ...
        top_5_concentration = (
            (top_5_spend / total_spend * 100) if total_spend > 0 else 0
        )
        top_10_concentration = (
            (top_10_spend / total_spend * 100) if total_spend > 0 else 0
        )
        ...
        for i, s in enumerate(suppliers):
            ...
            cumulative_pct = (
                pareto[i]["cumulative_percentage"] if i < len(pareto) else 100
            )
