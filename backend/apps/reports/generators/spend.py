...
class SpendAnalysisGenerator(BaseReportGenerator):
    ...
    def generate(self) -> dict:
        ...
        trend_analysis = {
            ...
            "avg_monthly_spend": (
                sum(m["amount"] for m in monthly_trend) / len(monthly_trend)
                if monthly_trend
                else 0
            ),
            "highest_month": (
                max(monthly_trend, key=lambda x: x["amount"]) if monthly_trend else None
            ),
            "lowest_month": (
                min(monthly_trend, key=lambda x: x["amount"]) if monthly_trend else None
            ),
        }
