...
class StratificationReportGenerator(BaseReportGenerator):
    ...
    def generate(self) -> dict:
        ...
        try:
            strategic_drilldown = self.analytics.get_stratification_segment_drilldown(
                "Strategic"
            )
        except Exception:
        ...
        return {
            ...
            "summary": {
                ...
                "most_fragmented_suppliers": summary.get(
                    "most_fragmented_suppliers", 0
                ),
    ...
    def _generate_recommendations(self, summary, segments, spend_bands, total_spend):
        ...
        strategic_segment = next(
            (s for s in segments if s.get("segment") == "Strategic"), None
        )
        ...
        tactical_segment = next(
            (s for s in segments if s.get("segment") == "Tactical"), None
        )
        if tactical_segment:
