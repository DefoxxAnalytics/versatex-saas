...
class SeasonalityReportGenerator(BaseReportGenerator):
    ...
    def _calculate_seasonal_indices(self, monthly_data):
        ...
        overall_average = (
            sum(monthly_averages) / len(monthly_averages) if monthly_averages else 0
        )
    ...
    def _analyze_peak_trough(self, monthly_data, seasonal_indices):
        ...
        sorted_indices = sorted(
            seasonal_indices, key=lambda x: x["index"], reverse=True
        )
        ...
        variance = sum((idx - mean_index) ** 2 for idx in indices_values) / len(
            indices_values
        )
        ...
        for cat in category_seasonality:
            if cat.get("savings_potential", 0) > 0:
                ...
                if strength > 30:
                    ...
                    description = (
                        f"Lock in annual pricing to reduce seasonal volatility"
                    )
