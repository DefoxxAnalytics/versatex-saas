...
class TestPreferencesMasking:
    ...
    def test_mask_helper_direct(self):
        ...
        assert UserProfile.mask_preferences({'aiApiKey': TEST_KEY})['aiApiKey'] == '****' + TEST_KEY[-4:]
...
class TestPreferencesPersistence:
    ...
    def test_ai_settings_reach_ai_service_init(self, admin_user, auth_client, saved_api_key):
        ...
        def fake_init(self, organization=None, filters=None, use_external_ai=False,
                      ai_provider='anthropic', api_key=None, **kwargs):
        ...
        with patch('apps.analytics.views.AIInsightsService.__init__', fake_init):
            _get_ai_service(FakeRequest(admin_user), organization=admin_user.profile.organization)
    ...
    def test_allowlist_contains_6_ai_fields(self):
        for key in ('useExternalAI', 'aiProvider', 'aiApiKey',
