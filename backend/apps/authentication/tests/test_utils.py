...
class TestLogAction:
    ...
    def test_hashes_user_agent(self, admin_user, mock_request):
        ...
        log = log_action(
            user=admin_user, action="login", resource="auth", request=mock_request
        )
    ...
    def test_returns_none_without_profile(self, mock_request):
        ...
        log = log_action(
            user=user, action="test", resource="test", request=mock_request
        )
    ...
    def test_log_without_request(self, admin_user):
        ...
        log = log_action(
            user=admin_user, action="update", resource="settings", resource_id="1"
        )
...
class TestLogSecurityEvent:
    ...
    def test_logs_with_details(self, mock_request):
        ...
        with patch("apps.authentication.utils.logger") as mock_logger:
            log_security_event(
                event_type="lockout", request=mock_request, details={"attempts": 5}
            )
    ...
    def test_logs_without_details(self, mock_request):
        ...
        with patch("apps.authentication.utils.logger") as mock_logger:
            log_security_event(event_type="suspicious_activity", request=mock_request)
