...
class TestOrganization:
    ...
    def test_create_organization(self):
        ...
        org = Organization.objects.create(
            name="Test Org", slug="test-org", description="A test organization"
        )
...
class TestUserProfile:
    ...
    def test_role_choices(self, organization):
        ...
        for role, _ in UserProfile.ROLE_CHOICES:
            profile = UserProfile(user=user, organization=organization, role=role)
...
class TestAuditLog:
    ...
    def test_audit_log_action_choices(self, organization):
        ...
        for action, _ in AuditLog.ACTION_CHOICES:
            log = AuditLog.objects.create(
                user=user, organization=organization, action=action, resource="test"
            )
    ...
    def test_audit_log_ordering(self, organization):
        ...
        log1 = AuditLog.objects.create(
            user=user, organization=organization, action="login", resource="session"
        )
        log2 = AuditLog.objects.create(
            user=user, organization=organization, action="logout", resource="session"
        )
        log3 = AuditLog.objects.create(
            user=user, organization=organization, action="view", resource="dashboard"
        )
