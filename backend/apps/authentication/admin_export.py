...
def build_org_zip(org):
    ...
    with zipfile.ZipFile(buf, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
        counts["suppliers"] = _write(
            zf, org, "suppliers.csv", SUPPLIER_COLUMNS, _supplier_rows(org)
        )
        counts["categories"] = _write(
            zf, org, "categories.csv", CATEGORY_COLUMNS, _category_rows(org)
        )
        counts["transactions"] = _write(
            zf, org, "transactions.csv", TRANSACTION_COLUMNS, _transaction_rows(org)
        )
        counts["prs"] = _write(
            zf, org, "purchase_requisitions.csv", PR_COLUMNS, _pr_rows(org)
        )
        counts["pos"] = _write(
            zf, org, "purchase_orders.csv", PO_COLUMNS, _po_rows(org)
        )
        counts["grs"] = _write(zf, org, "goods_receipts.csv", GR_COLUMNS, _gr_rows(org))
        counts["invoices"] = _write(
            zf, org, "invoices.csv", INVOICE_COLUMNS, _invoice_rows(org)
        )
        counts["contracts"] = _write(
            zf, org, "contracts.csv", CONTRACT_COLUMNS, _contract_rows(org)
        )
        counts["policies"] = _write(
            zf, org, "spending_policies.csv", POLICY_COLUMNS, _policy_rows(org)
        )
        counts["violations"] = _write(
            zf, org, "policy_violations.csv", VIOLATION_COLUMNS, _violation_rows(org)
        )
        zf.writestr(
...
def _transaction_rows(org):
    qs = (
        org.transactions.select_related("supplier", "category")
        .all()
        .order_by("date", "id")
    )
...
def _pr_rows(org):
    qs = (
        org.purchase_requisitions.select_related("supplier_suggested", "category")
        .all()
        .order_by("pr_number")
    )
    for pr in qs:
        yield {
            ...
            "supplier_suggested": (
                pr.supplier_suggested.name if pr.supplier_suggested_id else ""
            ),
...
def _po_rows(org):
    qs = (
        org.purchase_orders.select_related("supplier", "category", "requisition")
        .all()
        .order_by("po_number")
    )
    for po in qs:
...
def _invoice_rows(org):
    qs = (
        org.invoices.select_related("supplier", "purchase_order", "goods_receipt")
        .all()
    ...
    for inv in qs:
        yield {
            ...
            "payment_terms_days": (
                inv.payment_terms_days if inv.payment_terms_days is not None else ""
            ),
            "status": inv.status,
...
def _contract_rows(org):
    qs = (
        org.contracts.select_related("supplier")
    ...
    for c in qs:
        yield {
            ...
            "renewal_notice_days": (
                c.renewal_notice_days if c.renewal_notice_days is not None else ""
            ),
            "status": c.status,
...
def _violation_rows(org):
    qs = (
        org.policy_violations.select_related("transaction", "policy")
        .all()
        .order_by("created_at")
    )
    for v in qs:
