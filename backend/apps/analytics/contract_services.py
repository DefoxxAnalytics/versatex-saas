...
from apps.procurement.models import Contract, Transaction, Supplier, Category
...
class ContractAnalyticsService:
    ...
    def get_contract_detail(self, contract_id):
        ...
        monthly_spend = self.transactions.filter(
            supplier_id=contract.supplier_id,
            date__gte=contract.start_date
        ).annotate(
    ...
    def get_expiring_contracts(self, days=90):
        ...
        expiring = self.contracts.filter(
            status='active',
            end_date__lte=expiring_date,
            end_date__gte=today
    ...
    def get_contract_performance(self, contract_id):
        ...
        # Calculate metrics
        total_spend = transactions.aggregate(total=Sum('amount'))['total'] or Decimal('0')
        ...
        variance_percentage = (variance / expected_monthly * 100) if expected_monthly > 0 else 0

        return {
            'contract_id': contract_id,
            'total_spend': float(total_spend),
            'transaction_count': transaction_count,
            'utilization_percentage': round(float(total_spend / contract.total_value * 100) if contract.total_value > 0 else 0, 1),
    ...
    def get_savings_opportunities(self):
        ...
        supplier_spend = self.transactions.filter(
            date__gte=today - timedelta(days=365)  # Last year
        ).exclude(
            supplier_id__in=contract_supplier_ids
        ).values(
            ...
            total=Sum('amount'),
            count=Count('id')
        ).filter(total__gte=10000).order_by('-total')  # Only significant spend
        ...
        for supplier in supplier_spend[:10]:  # Top 10 opportunities
            ...
            opportunities.append({
                ...
                'severity': 'high' if float(supplier['total']) > 50000 else 'medium',
    ...
    def get_renewal_recommendations(self):
        ...
        for contract in upcoming:
            ...
            utilization = float(actual_spend / contract.total_value * 100) if contract.total_value > 0 else 0
            ...
            trend = 'increasing' if recent_spend > previous_spend else 'decreasing' if recent_spend < previous_spend else 'stable'
            ...
            if utilization > 100:
                ...
                action = 'evaluate'
                recommendation = 'Evaluate need - consider not renewing or consolidating'
                priority = 'low'
        ...
        recommendations.sort(key=lambda x: (priority_order.get(x['priority'], 3), x['days_remaining']))
    ...
    def get_contract_vs_actual_spend(self, contract_id=None):
        ...
        for contract in contracts.select_related('supplier'):
            ...
            variance_percentage = (variance / float(expected_spend) * 100) if expected_spend > 0 else 0
            ...
            comparisons.append({
                ...
                'days_remaining': (contract.end_date - today).days if contract.end_date > today else 0
            })
