...
class LLMRequestLog(models.Model):
    ...
    request_type = models.CharField(
        max_length=50, choices=REQUEST_TYPE_CHOICES, db_index=True
    )
    model_used = models.CharField(max_length=100, db_index=True)
    model_tier = models.CharField(
        max_length=20, choices=MODEL_TIER_CHOICES, null=True, blank=True
    )
    ...
    cache_hit = models.BooleanField(
        default=False, help_text="Whether response was served from semantic/Redis cache"
    )
    prompt_cache_read_tokens = models.IntegerField(
        default=0, help_text="Anthropic prompt cache: tokens read from cache"
    )
    prompt_cache_write_tokens = models.IntegerField(
        default=0, help_text="Anthropic prompt cache: tokens written to cache"
    )

    validation_passed = models.BooleanField(
        default=True, help_text="Whether LLM response passed hallucination validation"
    )
    ...
    def cost_savings_from_cache(self) -> Decimal:
        ...
        if self.prompt_cache_read_tokens > 0:
            ...
            savings = (
                base_cost_per_token - cached_cost
    ...
    def get_usage_summary(cls, organization_id: int, days: int = 30) -> dict:
        ...
        logs = cls.objects.filter(
            organization_id=organization_id, created_at__gte=cutoff
        )
        ...
        return {
            ...
            "total_tokens": (totals["total_input_tokens"] or 0)
            + (totals["total_output_tokens"] or 0),
            "avg_latency_ms": round(totals["avg_latency"] or 0, 1),
            "cache_hit_rate": round(
                (totals["cache_hits"] or 0)
                / max(totals["total_requests"] or 1, 1)
                * 100,
                1,
            ),
...
class SemanticCache(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="semantic_cache_entries"
    )
    ...
    def get_cache_stats(cls, organization_id: int) -> dict:
        ...
        entries = cls.objects.filter(
            organization_id=organization_id, expires_at__gt=timezone.now()
        )
        ...
        by_type = list(
            ...
            .annotate(count=Count("id"), hits=Sum("hit_count"))
            .order_by("-hits")
        )
...
class EmbeddedDocument(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="embedded_documents"
    )
    ...
    title = models.CharField(max_length=255, help_text="Document title for display")
    content = models.TextField(
        help_text="Full document content (will be chunked if too long)"
    )
    content_hash = models.CharField(
        max_length=64, db_index=True, help_text="SHA-256 hash for deduplication"
    )
    ...
    is_active = models.BooleanField(
        default=True, help_text="Soft delete - inactive docs excluded from search"
    )
        ...
        if source_model and source_id:
            ...
            if existing:
                ...
                if (
                    PGVECTOR_AVAILABLE
                    and embedding
                    and hasattr(existing, "content_embedding")
                ):
    ...
    def get_document_stats(cls, organization_id: int) -> dict:
        ...
        docs = cls.objects.filter(organization_id=organization_id, is_active=True)

        by_type = list(
            docs.values("document_type").annotate(count=Count("id")).order_by("-count")
        )
...
class InsightFeedback(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, related_name="insight_feedback"
    )
    ...
    action_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="insight_actions"
    )
    action_notes = models.TextField(blank=True)

    # Outcome tracking (updated later after implementation)
    outcome = models.CharField(
        max_length=20, choices=OUTCOME_CHOICES, default="pending", db_index=True
    )
    ...
    def savings_accuracy(self) -> float | None:
        ...
        if (
            self.actual_savings
            and self.predicted_savings
            and self.predicted_savings > 0
        ):
