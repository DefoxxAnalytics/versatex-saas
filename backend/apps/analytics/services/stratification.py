...
class StratificationAnalyticsService(BaseAnalyticsService):
    ...
    def get_spend_stratification(self):
        ...
        """
        categories = self.transactions.values("category__name", "category_id").annotate(
        ...
        median_suppliers = (
            sorted(supplier_counts)[len(supplier_counts) // 2] if supplier_counts else 0
        )
    ...
    def get_detailed_stratification(self):
        ...
        band_data = list(
            self.transactions.values("spend_band").annotate(
                total_spend=Sum("amount"),
                transaction_count=Count("id"),
                supplier_count=Count("supplier", distinct=True),
            )
        ...
        for band_def in SPEND_BANDS:
            ...
            band_info = band_map.get(
                band_name,
                {"total_spend": 0, "transaction_count": 0, "supplier_count": 0},
            )
            ...
            percent_of_total = (
                (band_spend / total_spend * 100) if total_spend > 0 else 0
            )
        ...
        for seg_def in SEGMENTS:
            ...
            percent_of_total = (
                (segment_spend / total_spend * 100) if total_spend > 0 else 0
            )
            ...
            if seg_def["min"] == 0:
                ...
                spend_range = (
                    f"${int(seg_def['min'] / 1000)}K-${int(seg_def['max'] / 1000)}K"
                )
        ...
        strategic_bands = len(
            [
                ...
                if b["strategic_importance"] in ["Strategic", "Critical"]
                and b["suppliers"] > 0
            ]
        )
        high_risk_bands = len(
            [b for b in spend_bands if b["risk_level"] == "High" and b["suppliers"] > 0]
        )
        complex_bands = len([b for b in spend_bands if b["suppliers"] >= 50])

        # Find highest impact band
        highest_impact = (
            max(spend_bands, key=lambda b: b["percent_of_total"])
            if spend_bands
            else None
        )
        ...
        most_fragmented = (
            max(spend_bands, key=lambda b: b["suppliers"]) if spend_bands else None
        )

        # Average suppliers per band
        total_suppliers_in_bands = sum(b["suppliers"] for b in spend_bands)
        avg_suppliers_per_band = (
            total_suppliers_in_bands / len(spend_bands) if spend_bands else 0
        )

        # Strategic segment for risk assessment
        strategic_segment = next(
            (s for s in segments if s["segment"] == "Strategic"), None
        )
        strategic_concentration = (
            strategic_segment["percent_of_total"] if strategic_segment else 0
        )
        ...
        if highest_impact and highest_impact["percent_of_total"] > 50:
            recommendations.append(
                f"Implement supplier diversification strategy within the {highest_impact['label']} band"
            )
            recommendations.append(
                "Evaluate supplier consolidation opportunities to reduce administrative burden"
            )
        ...
        for band in spend_bands:
            if (
                band["suppliers"] > 100
                and "K" in band["label"]
                and "M" not in band["label"]
            ):
                recommendations.append(
                    f"{band['label']}: Consider supplier consolidation to improve efficiency"
                )
            if (
                band["percent_of_total"] > 10
                and band["suppliers"] < 5
                and band["suppliers"] > 0
            ):
                recommendations.append(
                    f"{band['label']}: High concentration with few suppliers - diversification recommended"
                )
        ...
        return {
            "summary": {
                ...
                "highest_impact_band": (
                    highest_impact["label"] if highest_impact else "N/A"
                ),
                "highest_impact_percent": (
                    highest_impact["percent_of_total"] if highest_impact else 0
                ),
                "most_fragmented_band": (
                    most_fragmented["label"] if most_fragmented else "N/A"
                ),
                "most_fragmented_suppliers": (
                    most_fragmented["suppliers"] if most_fragmented else 0
                ),
    ...
    def get_stratification_segment_drilldown(self, segment_name):
        ...
        segment_transactions = self.transactions.filter(
            spend_band__in=segment_band_names
        )
        ...
        # Calculate totals
        total_spend = float(
            segment_transactions.aggregate(total=Sum("amount"))["total"] or 0
        )
        transaction_count = segment_transactions.count()

        # Get supplier breakdown
        suppliers_data = list(
            segment_transactions.values("supplier__name", "supplier_id")
            .annotate(
                total_spend=Sum("amount"),
                transaction_count=Count("id"),
        ...
        avg_spend_per_supplier = (
            total_spend / supplier_count if supplier_count > 0 else 0
        )
        ...
        suppliers = [
            {
                ...
                "percent_of_segment": round(
                    (
                        (float(s["total_spend"] or 0) / total_spend * 100)
                        if total_spend > 0
                        else 0
                    ),
                    2,
        ...
        subcategories_data = list(
            segment_transactions.values("subcategory")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        subcategories = [
            {
                ...
                "percent_of_segment": round(
                    (
                        (float(sub["spend"] or 0) / total_spend * 100)
                        if total_spend > 0
                        else 0
                    ),
                    2,
        ...
        locations_data = list(
            segment_transactions.values("location")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        locations = [
            {
                ...
                "percent_of_segment": round(
                    (
                        (float(loc["spend"] or 0) / total_spend * 100)
                        if total_spend > 0
                        else 0
                    ),
                    2,
    ...
    def get_stratification_band_drilldown(self, band_name):
        ...
        # Calculate totals
        total_spend = float(
            band_transactions.aggregate(total=Sum("amount"))["total"] or 0
        )
        transaction_count = band_transactions.count()

        # Get supplier breakdown
        suppliers_data = list(
            band_transactions.values("supplier__name", "supplier_id")
            .annotate(
                total_spend=Sum("amount"),
                transaction_count=Count("id"),
                subcategory_count=Count("subcategory", distinct=True),
        ...
        # Get subcategory breakdown (top 10)
        subcategories_data = list(
            band_transactions.values("subcategory")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        # Get location breakdown (top 10)
        locations_data = list(
            band_transactions.values("location")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
