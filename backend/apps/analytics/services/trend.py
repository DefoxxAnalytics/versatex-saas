...
class TrendConsolidationAnalyticsService(BaseAnalyticsService):
    ...
    def get_monthly_trend(self, months=12):
        ...
        """
        # v3.1 Phase 3 (X-3): timezone.now().date() instead of naive
        # datetime.now() so the cutoff respects the deployed timezone
        # boundary rather than naive server-local rollover.
        cutoff_date = timezone.now().date() - timedelta(days=months * 30)

        data = (
            self.transactions.filter(date__gte=cutoff_date)
