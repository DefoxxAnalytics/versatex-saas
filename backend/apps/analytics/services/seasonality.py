...
class SeasonalityAnalyticsService(BaseAnalyticsService):
    ...
    def get_detailed_seasonality_analysis(self, use_fiscal_year=True, year=None):
        ...
        for fiscal_month in range(1, 13):
            ...
            month_entry["average"] = (
                round(sum(year_values) / len(year_values), 2) if year_values else 0
            )
        ...
        for category_name, cat_transactions in category_data.items():
            ...
            for spend in cat_monthly_spend:
                index = (
                    (spend / avg_monthly_spend * 100) if avg_monthly_spend > 0 else 100
                )
                seasonal_indices.append(round(index, 2))

            # Calculate peak spend percentage
            peak_spend_percentage = (
                (max_spend / total_spend * 100) if total_spend > 0 else 0
            )

            # Calculate seasonality strength (coefficient of variation)
            mean_index = sum(seasonal_indices) / len(seasonal_indices)
            variance = sum((idx - mean_index) ** 2 for idx in seasonal_indices) / len(
                seasonal_indices
            )
            ...
            savings_potential = (
                total_spend * (peak_spend_percentage / 100) * savings_rate
            )
            ...
            category_seasonality.append(
                {
                    ...
                    "category_id": (
                        cat_transactions[0]["category_id"] if cat_transactions else None
                    ),
        ...
        opportunities_found = len(
            [c for c in category_seasonality if c["seasonality_strength"] > 15]
        )
        high_impact_count = len(
            [c for c in category_seasonality if c["impact_level"] == "High"]
        )
        total_savings_potential = sum(
            c["savings_potential"] for c in category_seasonality
        )
        avg_yoy_growth = (
            sum(c["yoy_growth"] for c in category_seasonality)
            / len(category_seasonality)
    ...
    def get_seasonality_category_drilldown(self, category_id, use_fiscal_year=True):
        ...
        cat_transactions = list(
            self.transactions.filter(category_id=category_id).values(
            ...
            suppliers.append(
                {
                    ...
                    "percent_of_category": round(
                        (sup_total / total_spend * 100) if total_spend > 0 else 0, 2
                    ),
