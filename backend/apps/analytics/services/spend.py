...
class SpendAnalyticsService(BaseAnalyticsService):
    ...
    def get_spend_by_category(self):
        ...
        """
        data = (
            self.transactions.values("category__name", "category_id")
    ...
    def get_detailed_category_analysis(self):
        ...
        subcategory_rows = list(
            self.transactions.values("category_id", "subcategory")
            .annotate(
                spend=Sum("amount"),
                transaction_count=Count("id"),
                supplier_count=Count("supplier", distinct=True),
        ...
        for cat in categories:
            ...
            top_subcategory_name = (
                top_subcategory["subcategory"] if top_subcategory else "N/A"
            )
            top_subcategory_spend = (
                float(top_subcategory["spend"]) if top_subcategory else 0
            )
            ...
            concentration = (
                (top_subcategory_spend / total_spend * 100) if total_spend > 0 else 0
            )
            ...
            result.append(
                {
                    ...
                    "percent_of_total": round(
                        (
                            (total_spend / total_all_spend * 100)
                    ...
                    "avg_spend_per_supplier": (
                        round(total_spend / supplier_count, 2)
                        if supplier_count > 0
                        else 0
                    ),
    ...
    def get_detailed_supplier_analysis(self):
            ...
            supplier_list.append(
                {
                    ...
                    "avg_transaction": (
                        round(spend / sup["transaction_count"], 2)
                        if sup["transaction_count"]
                        else 0
                    ),
        ...
        return {
            "summary": {
                ...
                "top_supplier_spend": (
                    top_supplier["total_spend"] if top_supplier else 0
                ),
            },
    ...
    def get_supplier_drilldown(self, supplier_id):
        ...
        # Basic metrics
        total_spend = float(
            supplier_transactions.aggregate(total=Sum("amount"))["total"] or 0
        )
        transaction_count = supplier_transactions.count()
        avg_transaction = (
            total_spend / transaction_count if transaction_count > 0 else 0
        )
        ...
        # Category breakdown
        categories = list(
            supplier_transactions.values("category__name")
        ...
        subcategories = list(
            supplier_transactions.values("subcategory")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        locations = list(
            supplier_transactions.values("location")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        return {
            ...
            "date_range": {
                "min": (
                    date_agg["min_date"].isoformat() if date_agg["min_date"] else None
                ),
                "max": (
                    date_agg["max_date"].isoformat() if date_agg["max_date"] else None
                ),
            },
    ...
    def get_category_drilldown(self, category_id):
        ...
        total_spend = float(
            category_transactions.aggregate(total=Sum("amount"))["total"] or 0
        )
        ...
        suppliers = list(
            category_transactions.values("supplier__name", "supplier_id")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        supplier_data = [
            {
                ...
                "percent_of_total": round(
                    (
                        (float(sup["spend"] or 0) / total_spend * 100)
                        if total_spend > 0
                        else 0
                    ),
                    2,
        ...
        subcategories = list(
            category_transactions.values("subcategory")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")
        )
        ...
        locations = list(
            category_transactions.values("location")
            .annotate(spend=Sum("amount"), transaction_count=Count("id"))
            .order_by("-spend")[:10]
        )
        ...
        recent_txns = list(
            category_transactions.select_related("supplier").order_by("-date")[:10]
        )
        recent_transactions = [
            {
                "id": txn.id,
