...
class ParetoTailAnalyticsService(BaseAnalyticsService):
    ...
    def get_pareto_analysis(self):
        ...
        """
        suppliers = (
            self.transactions.values("supplier__name", "supplier_id")
    ...
    def get_tail_spend_analysis(self, threshold_percentage=20):
        ...
        threshold_amount = (
            total_spend * Decimal(str(threshold_percentage)) / Decimal("100")
        )
        ...
        return {
            ...
            "tail_percentage": round(
                (cumulative / total_spend * 100) if total_spend > 0 else 0, 2
            ),
        }
    ...
    def get_detailed_tail_spend(self, threshold=50000):
        ...
        supplier_spend = {
            s["supplier_id"]: float(s["total_spend"] or 0) for s in suppliers
        }
            ...
            category_analysis.append(
                {
                    ...
                    "tail_percentage": round(
                        (
                            (tail_cat_spend / total_cat_spend * 100)
                            if total_cat_spend > 0
                            else 0
                        ),
                        2,
                    ),
                    "vendor_percentage": round(
                        (
                            (tail_vendors_count / total_vendors_count * 100)
        ...
        if multi_tail_supplier_ids:
            for row in (
                self.transactions.filter(supplier_id__in=multi_tail_supplier_ids)
                ...
                multi_category.append(
                    {
                        ...
                        "savings_potential": round(
                            float(sup["total_spend"] or 0) * 0.15, 2
                        ),  # 15% consolidation savings
                    }
                )
        ...
        if qualifying_category_ids:
            for row in (
                self.transactions.filter(category_id__in=qualifying_category_ids)
                .values("category_id", "supplier__name")
                .annotate(spend=Sum("amount"))
                ...
                category_consolidation.append(
                    {
                        ...
                        "savings_potential": round(
                            cat["tail_spend"] * 0.10, 2
                        ),  # 10% consolidation savings
                    }
                )
        ...
        location_data = defaultdict(
            lambda: {"tail_vendors": set(), "total_vendors": set(), "tail_spend": 0}
        )

        loc_transactions = list(
            self.transactions.values("supplier_id", "location").annotate(
                spend=Sum("amount")
            )
        )
        ...
        if qualifying_locations:
            for row in (
                self.transactions.filter(location__in=qualifying_locations)
                .values("location", "supplier__name")
                .annotate(spend=Sum("amount"))
        ...
        total_opportunities = (
            len(multi_category)
            + len(category_consolidation)
            + len(geographic_consolidation)
        )
        ...
        savings_by_type = {
            "Multi-Category Vendors": sum(
                m["savings_potential"] for m in multi_category
            ),
            "Category Consolidation": sum(
                c["savings_potential"] for c in category_consolidation
            ),
            "Geographic Consolidation": sum(
                g["savings_potential"] for g in geographic_consolidation
            ),
        }
        top_type = (
            max(savings_by_type, key=savings_by_type.get) if savings_by_type else "N/A"
        )
    ...
    def get_tail_spend_category_drilldown(self, category_id, threshold=50000):
        ...
        suppliers = list(
            ...
            .values("supplier__name", "supplier_id")
        ...
        global_supplier_spend = dict(
            self.transactions.values("supplier_id")
            .annotate(total=Sum("amount"))
            ...
            vendors.append(
                {
                    ...
                    "percent_of_category": round(
                        (spend / total_spend * 100) if total_spend > 0 else 0, 2
                    ),
                }
            )
        ...
        if tail_vendor_count >= 5:
            recommendations.append(
                f"Consider consolidating {tail_vendor_count} tail vendors into fewer strategic suppliers"
            )
        if tail_percentage > 30:
            recommendations.append(
                f"High tail spend ({tail_percentage:.1f}%) suggests opportunity for preferred vendor program"
            )
        if any(v["is_tail"] and v["percent_of_category"] > 10 for v in vendors):
            recommendations.append(
                "Some tail vendors have significant category share - evaluate for strategic partnership"
            )
    ...
    def get_tail_spend_vendor_drilldown(self, supplier_id, threshold=50000):
        ...
        total_agg = self.transactions.filter(supplier_id=supplier_id).aggregate(
            total_spend=Sum("amount"), transaction_count=Count("id")
        )
        ...
        category_data = [
            {
                ...
                "percent_of_vendor": round(
                    (
                        (float(cat["spend"] or 0) / total_spend * 100)
                        if total_spend > 0
                        else 0
                    ),
                    2,
        ...
        monthly_txns = (
            self.transactions.filter(
                supplier_id=supplier_id, date__gte=twelve_months_ago
            )
