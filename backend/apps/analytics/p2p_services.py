...
from django.db.models.functions import Coalesce, ExtractMonth, TruncMonth, TruncWeek
...
class P2PAnalyticsService:
    ...
    @staticmethod
    def _avg_days_to_pay(paid_invoices, return_eligible=False):
        """
        ...
        v3.1 Phase 2 (AN-H2): when ``return_eligible=True``, also returns the
        list of invoices that passed the data-quality gate, so callers can
        compute companion metrics (on_time, etc.) over the same filtered set
        without re-iterating the queryset (which would force a second DB
        round-trip if the QuerySet hasn't been cached).

        Returns:
            tuple: (avg_days, sample_size) — default
            tuple: (avg_days, sample_size, eligible_list) — when
        ...
        days = [(inv.paid_date - inv.invoice_date).days for inv in eligible]
        avg = (sum(days) / len(days)) if days else 0
        if return_eligible:
            return avg, len(days), eligible
        return avg, len(days)
    ...
    def get_p2p_cycle_overview(self):
        ...
        latest_po_per_pr = (
            PurchaseOrder.objects.filter(requisition=OuterRef("pk"))
        ...
        prs_qs = prs_qs.annotate(latest_po_date=Subquery(latest_po_per_pr)).filter(
            latest_po_date__gte=F("approval_date")
        )
        ...
        avg_pr_to_po = (
            pr_agg["avg_delta"].total_seconds() / 86400 if pr_agg["avg_delta"] else 0
        )
        pr_to_po_sample = pr_agg["sample"] or 0

        # ---- Stage 2: PO → GR --------------------------------------------------
        latest_gr_per_po = (
            GoodsReceipt.objects.filter(purchase_order=OuterRef("pk"))
            .order_by("-received_date", "-created_at")
            .values("received_date")[:1]
        )
        ...
        pos_qs = pos_qs.annotate(latest_gr_date=Subquery(latest_gr_per_po)).filter(
            latest_gr_date__gte=F("sent_date")
        )
        ...
        avg_po_to_gr = (
            po_agg["avg_delta"].total_seconds() / 86400 if po_agg["avg_delta"] else 0
        )
        po_to_gr_sample = po_agg["sample"] or 0

        # ---- Stage 3: GR → Invoice --------------------------------------------
        latest_inv_per_gr = (
            Invoice.objects.filter(goods_receipt=OuterRef("pk"))
        ...
        grs_qs = (
            ...
            .distinct()
            .annotate(latest_inv_date=Subquery(latest_inv_per_gr))
        ...
        avg_gr_to_inv = (
            gr_agg["avg_delta"].total_seconds() / 86400 if gr_agg["avg_delta"] else 0
        )
        ...
        avg_inv_to_pay = (
            inv_agg["avg_delta"].total_seconds() / 86400 if inv_agg["avg_delta"] else 0
        )
    ...
    def get_cycle_time_trends(self, months=12):
        ...
        paid_invoices = (
            ...
            .values("month")
            .annotate(
                avg_days=Avg(F("paid_date") - F("invoice_date")), count=Count("id")
            )
            .order_by("month")
        )
    ...
    def get_cycle_time_by_category(self):
        ...
        v3.1 Phase 2 (AN-M2): single grouped DB aggregation. Previously
        materialised every paid-and-PO-linked invoice in the org and
        grouped in Python; now a single GROUP BY with Avg/Sum/Count
        annotations stays O(category_count) regardless of invoice volume.
        Same data-quality gate (paid_date >= invoice_date) on the avg.
        """
        ...
        rows = invoices.values("purchase_order__category__name").annotate(
            avg_delta=Avg(
                F("paid_date") - F("invoice_date"),
                filter=Q(paid_date__gte=F("invoice_date")),
            ),
            spend=Sum("invoice_amount"),
            count=Count("id"),
        )

        result = []
        for row in rows:
            cat_name = row["purchase_order__category__name"] or "Uncategorized"
            delta = row["avg_delta"]
            avg_days = (
                delta.total_seconds() / 86400.0 if isinstance(delta, timedelta) else 0
            )
            result.append(
                {
                    "category": cat_name,
    ...
    def get_cycle_time_by_supplier(self):
        ...
        v3.1 Phase 2 (AN-M2): grouped DB aggregation — see get_cycle_time_
        by_category. on_time numerator now uses ``paid_date <= due_date``
        directly at SQL (NULL-safe — invoices with NULL due_date are
        excluded), matching the Python ``inv.days_overdue == 0`` semantics
        without raising on null due dates.
        """
        invoices = Invoice.objects.filter(
            organization=self.organization,
            status="paid",
        ...
        rows = invoices.values("supplier__name").annotate(
            avg_delta=Avg(
                F("paid_date") - F("invoice_date"),
                filter=Q(paid_date__gte=F("invoice_date")),
            ),
            ...
            on_time=Count(
                "id",
                filter=Q(due_date__isnull=False, paid_date__lte=F("due_date")),
            ),
        )

        result = []
        for row in rows:
            supplier_name = row["supplier__name"] or "Unknown"
            delta = row["avg_delta"]
            avg_days = (
                delta.total_seconds() / 86400.0 if isinstance(delta, timedelta) else 0
            )
            count = row["count"] or 0
            on_time_rate = (row["on_time"] / count * 100) if count else 0
            result.append(
                {
    ...
    def get_stage_drilldown(self, stage):
        ...
        if stage == "pr_to_po":
            prs = (
                ...
                .select_related("supplier_suggested")
                .prefetch_related("purchase_orders")[:100]
            )
                ...
                slowest_docs.append(
                    {
                        ...
                        "supplier_name": (
                            pr.supplier_suggested.name
                            if pr.supplier_suggested
                            else "N/A"
                        ),
                        "days_in_stage": days,
                        "amount": float(amt),
            ...
            pos = (
                ...
                .prefetch_related("goods_receipts")[:100]
            )
            ...
            grs = (
                GoodsReceipt.objects.filter(
                    organization=self.organization, invoices__isnull=False
                )
                .select_related("purchase_order", "purchase_order__supplier")
                .prefetch_related("invoices")[:100]
            )
                ...
                slowest_docs.append(
                    {
                        ...
                        "supplier_name": (
                            ...
                            if gr.purchase_order and gr.purchase_order.supplier
                            else "N/A"
                        ),
                        "days_in_stage": days,
                        "amount": float(amt),
            ...
            invoices = Invoice.objects.filter(
                organization=self.organization, status="paid", paid_date__isnull=False
    ...
    def get_bottleneck_analysis(self):
        ...
        for stage_name, stage_data in stages.items():
            variance = (
                (
                    (stage_data["avg_days"] - stage_data["target_days"])
                    / stage_data["target_days"]
                    * 100
                )
                if stage_data["target_days"] > 0
                else 0
            )
    ...
    def get_process_funnel(self, months=12):
        ...
        # v3.1 Phase 1 (AN-C3): every stage now respects the same date filter
        # as the PR stage. Previously PR was date-filtered while PO/GR/Invoice
        # used org-lifetime totals, so the funnel rendered nonsensical
        # conversion rates (e.g., 40 PRs → 500 POs) under any date selection.
        # Each stage uses its own creation/event-date column.
        pr_count = PurchaseRequisition.objects.filter(filters).count()
        pr_approved = PurchaseRequisition.objects.filter(
            filters, status__in=["approved", "converted_to_po"]
        ).count()

        po_qs = PurchaseOrder.objects.filter(organization=self.organization)
        po_count = self._apply_date_filters(po_qs, "created_date").count()

        gr_qs = GoodsReceipt.objects.filter(organization=self.organization)
        gr_count = self._apply_date_filters(gr_qs, "received_date").count()

        inv_qs = Invoice.objects.filter(organization=self.organization)
        inv_count = self._apply_date_filters(inv_qs, "invoice_date").count()

        inv_paid_qs = Invoice.objects.filter(
            organization=self.organization,
            status="paid",
        )
        inv_paid = self._apply_date_filters(inv_paid_qs, "invoice_date").count()
    ...
    def get_matching_overview(self):
        ...
        total_amount = invoices.aggregate(total=Sum("invoice_amount"))[
            "total"
        ] or Decimal("0")

        # Match status breakdown
        match_stats = invoices.values("match_status").annotate(
            count=Count("id"), amount=Sum("invoice_amount")
        )
        ...
        exception_amount = exceptions.aggregate(total=Sum("invoice_amount"))[
            "total"
        ] or Decimal("0")
        ...
        # v3.1 Phase 3 (AN-TD4): values_list pulls only the two date columns
        # so we don't materialise full Invoice ORM objects. exception_resolved_at
        # is a tz-aware DateTimeField; .date() is called on each value to
        # coerce to a calendar date in the server's timezone (matching the
        # prior behavior — true per-tenant TZ-correct resolution would
        # require threading org timezone through, tracked separately).
        resolved_pairs = invoices.filter(
            has_exception=True,
            exception_resolved=True,
            exception_resolved_at__isnull=False,
        ).values_list("exception_resolved_at", "invoice_date")

        resolution_times = []
        for resolved_at, invoice_date in resolved_pairs:
            if resolved_at and invoice_date:
                days = (resolved_at.date() - invoice_date).days
                if days >= 0:
        ...
        three_way_pct = (
            round(three_way["count"] / total_count * 100, 1) if total_count > 0 else 0
        )
        two_way_pct = (
            round(two_way["count"] / total_count * 100, 1) if total_count > 0 else 0
        )
        exception_pct = (
            round(exception_count / total_count * 100, 1) if total_count > 0 else 0
        )

        three_way_pct_by_amount = (
            round(three_way["amount"] / total_amount_float * 100, 1)
            if total_amount_float > 0
            else 0
        )
        two_way_pct_by_amount = (
            round(two_way["amount"] / total_amount_float * 100, 1)
            if total_amount_float > 0
            else 0
        )
        exception_pct_by_amount = (
            round(float(exception_amount) / total_amount_float * 100, 1)
            if total_amount_float > 0
            else 0
        )
    ...
    def get_exceptions_by_type(self):
        ...
        exceptions = Invoice.objects.filter(
            organization=self.organization, has_exception=True
        )
        exceptions = self._apply_date_filters(exceptions, "invoice_date")

        by_type = (
            exceptions.values("exception_type")
            .annotate(
                count=Count("id"),
    ...
    def get_exceptions_by_supplier(self, limit=20):
        ...
        primary_type_subquery = (
            ...
            .order_by("-type_count", "exception_type")
            .values("exception_type")[:1]
        )

        by_supplier = (
            exceptions.values("supplier__id", "supplier__name")
            .annotate(
        ...
        supplier_ids = [
            item["supplier__id"] for item in by_supplier if item["supplier__id"]
        ]
        supplier_totals = {}
        if supplier_ids:
            totals = (
                Invoice.objects.filter(
                    organization=self.organization, supplier_id__in=supplier_ids
                )
                .values("supplier_id")
        ...
        for item in by_supplier:
            ...
            exception_rate = (
                (exception_count / total_invoices * 100) if total_invoices > 0 else 0
            )
    ...
    def get_price_variance_analysis(self):
        ...
        for inv in invoices[:50]:  # Limit for performance
            if inv.purchase_order and inv.purchase_order.total_amount > 0:
                variance_pct = (
                    (inv.invoice_amount - inv.purchase_order.total_amount)
                    / inv.purchase_order.total_amount
                    * 100
                )
    ...
    def get_quantity_variance_analysis(self):
        ...
        grs = GoodsReceipt.objects.filter(
            organization=self.organization, quantity_ordered__isnull=False
        ).select_related("purchase_order", "purchase_order__supplier")
        ...
        for gr in grs[:50]:  # Limit for performance
            if gr.quantity_ordered > 0:
                variance_pct = (
                    (gr.quantity_received - gr.quantity_ordered)
                    / gr.quantity_ordered
                    * 100
                )
                ...
                variances.append(
                    {
                        ...
                        "po_number": (
                            gr.purchase_order.po_number if gr.purchase_order else None
                        ),
                        "supplier_name": (
                            gr.purchase_order.supplier.name
                            if gr.purchase_order and gr.purchase_order.supplier
                        ...
                        "quantity_accepted": (
                            float(gr.quantity_accepted)
                            if gr.quantity_accepted
                            else None
                        ),
        ...
        return [
            {
                ...
                "exception_amount": (
                    float(inv.exception_amount) if inv.exception_amount else None
                ),
                "days_open": (
                    (today - inv.received_date).days
                    if inv.received_date
                    else (today - inv.invoice_date).days
                ),
                "po_number": (
                    inv.purchase_order.po_number if inv.purchase_order else None
                ),
    ...
    def get_invoice_match_detail(self, invoice_id):
        ...
        if inv.purchase_order:
            ...
            variance_detail = {
                ...
                "variance_percent": (
                    round(
                        float(
                            (inv.invoice_amount - po.total_amount)
                            / po.total_amount
                            * 100
                        ),
                        1,
        ...
        return {
            ...
            "exception_type_display": (
                dict(Invoice.EXCEPTION_TYPE_CHOICES).get(
    ...
    def get_aging_overview(self):
        ...
        # v3.1 Phase 2 (AN-H1): single-pass bucket dispatch. Materialise the
        # invoice queryset once, iterate once. Each invoice's days_outstanding
        # property is touched exactly once instead of N_BUCKETS times.
        buckets_state = [
            {
                "def": b,
                "count": 0,
        ...
        for inv in invoices_list:
            ...
            for bucket in buckets_state:
                bd = bucket["def"]
                if bd["min"] <= days <= bd["max"]:
                    bucket["count"] += 1
                    bucket["amount"] += inv.invoice_amount
                    break

        total_ap = sum((b["amount"] for b in buckets_state), Decimal("0"))
        total_overdue = sum(
            (b["amount"] for b in buckets_state if b["def"]["name"] != "Current"),
            Decimal("0"),
        )

        buckets_list = [
            {
                "bucket": b["def"]["display"],
                "count": b["count"],
                "amount": float(b["amount"]),
                "percentage": (
                    round(float(b["amount"]) / float(total_ap) * 100, 1)
                    if total_ap > 0
                    else 0
                ),
            }
            for b in buckets_state
        ]

        # Average days from invoice issuance to payment (legacy: DPO).
        # v3.1 Phase 2 (AN-H2): _avg_days_to_pay returns the data-quality-
        # filtered eligible list when asked, so the on-time numerator
        # iterates the same in-memory list rather than re-evaluating the
        # paid_invoices queryset (which previously round-tripped to the DB
        # twice — once for avg, once for on_time).
        paid_invoices = Invoice.objects.filter(
            organization=self.organization, status="paid", paid_date__isnull=False
        )
        avg_days_to_pay, sample_size, eligible = self._avg_days_to_pay(
            paid_invoices,
            return_eligible=True,
        )
        on_time = sum(1 for inv in eligible if inv.days_overdue == 0)
    ...
    def get_aging_by_supplier(self, limit=20):
        ...
        on_time_rates = {
            p["supplier_id"]: (
                p["on_time"] / p["total_paid"] * 100 if p["total_paid"] > 0 else 0
            )
        ...
        return [
            {
                ...
                "on_time_rate": (
                    round(item["on_time"] / item["total"] * 100, 1)
                    if item["total"] > 0
                    else 0
                ),
            }
    ...
    def get_cash_flow_forecast(self, weeks=4):
        ...
        for week in range(weeks):
            ...
            ).aggregate(amount=Sum("invoice_amount"), count=Count("id"))
    ...
    def get_pr_overview(self):
        ...
        # v3.1 Phase 2 (AN-M1): DB-side Avg(F() - F()) replaces the Python
        # loop that materialised every approved PR just to compute one
        # number. Same data-quality gate (delta >= 0) applied as filter so
        # negative-delta noise rows don't tilt the average.
        approval_agg = prs.filter(
            submitted_date__isnull=False,
            approval_date__isnull=False,
            approval_date__gte=F("submitted_date"),
        ).aggregate(avg_delta=Avg(F("approval_date") - F("submitted_date")))
        avg_approval = (
            approval_agg["avg_delta"].total_seconds() / 86400.0
            if approval_agg["avg_delta"]
            else 0
        )
        ...
        status_counts = prs.values("status").annotate(
            count=Count("id"), value=Sum("estimated_amount")
        )
        by_status = [
            {
                "status": item["status"],
        ...
        return [
            {
                ...
                "approval_rate": (
                    round(item["approved"] / item["count"] * 100, 1)
                    if item["count"] > 0
                    else 0
                ),
                "rejection_rate": (
                    round(item["rejected"] / item["count"] * 100, 1)
                    if item["count"] > 0
                    else 0
                ),
            }
    ...
    def get_pr_pending(self, limit=50):
        ...
        pending = (
            PurchaseRequisition.objects.filter(
                organization=self.organization, status="pending_approval"
            )
            .select_related("requested_by", "category")
            .order_by("-submitted_date")[:limit]
        )
        ...
        return [
            {
                ...
                "submitted_date": (
                    pr.submitted_date.isoformat() if pr.submitted_date else None
                ),
                "days_pending": (
                    (today - pr.submitted_date).days if pr.submitted_date else 0
                ),
    ...
    def get_po_overview(self):
        ...
        contract_value = contract_backed.aggregate(total=Sum("total_amount"))[
            "total"
        ] or Decimal("0")
        ...
        by_status = pos.values("status").annotate(
            count=Count("id"), value=Sum("total_amount")
        )
        ...
        contract_coverage_pct = (
            round(float(contract_value / total_value * 100), 1)
            if total_value > 0
            else 0
        )
    ...
    def get_po_leakage(self, limit=20):
        ...
        maverick_pos = PurchaseOrder.objects.filter(
            organization=self.organization, is_contract_backed=False
        ...
        for cat_name, data in by_category.items():
            ...
            maverick_pct = (
                float(data["maverick_amount"]) / cat_total * 100 if cat_total > 0 else 0
            )
    ...
    def get_po_amendment_analysis(self):
        ...
        avg_increase = (
            sum(value_increases) / len(value_increases) if value_increases else 0
        )
        avg_decrease = (
            sum(value_decreases) / len(value_decreases) if value_decreases else 0
        )

        return {
            "total_amended": total_amended,
            "amendment_rate": (
                round(total_amended / total_pos * 100, 1) if total_pos > 0 else 0
            ),
            "avg_amendments_per_po": (
                round(sum(po.amendment_count for po in amended_pos) / total_amended, 1)
                if total_amended > 0
                else 0
            ),
    ...
    def get_supplier_payments_overview(self):
        ...
        # v3.1 Phase 2 (AN-H2): single materialisation via return_eligible.
    ...
    def get_supplier_payments_scorecard(self, limit=50):
        ...
        v3.1 Phase 1 (AN-C4): single grouped aggregation replaces the prior
        N+1 (5 queries × 50 suppliers = 250 round-trips). Aggregates over
        ALL invoiced suppliers in the org, then ranks by score and returns
        top `limit` — improvement over the prior arbitrary-DB-order-then-
        sort-50 behavior that produced unstable top-50 rankings.

        Score formula preserved verbatim. Days-to-pay still treats 30 days
        as the aspirational benchmark (see docs/ACCURACY_AUDIT.md S4).
        """
        ...
        # Single grouped query: group by supplier_id, compute all metrics
        # via filtered aggregates. Same data-quality gate as
        # `_avg_days_to_pay` (paid_date >= invoice_date) so the scorecard's
        # avg matches the helper's value.
        agg = (
            Invoice.objects.filter(organization=self.organization)
            .values("supplier_id")
            .annotate(
                total_count=Count("id"),
                exception_count=Count("id", filter=Q(has_exception=True)),
                ap_balance=Sum(
                    "invoice_amount",
                    filter=Q(status__in=AP_OPEN_STATUSES),
                ),
                paid_sample=Count(
                    "id",
                    filter=Q(
                ...
                # v3.1 Phase 3 (AN-TD3): amount-weighted companion per
                # accuracy convention §1. A supplier with many small
                # on-time payments + one massive late payment looks
                # green on count-based on_time_rate but is materially
                # exposed; the by_amount view surfaces that.
                on_time_amount=Sum(
                    "invoice_amount",
                    filter=Q(
                        status="paid",
        ...
        # Build supplier_id -> name map in a single query (avoids per-row lookup).
        supplier_ids = [row["supplier_id"] for row in agg]
        supplier_names = dict(
        ...
        for row in agg:
            ...
            paid_sample = row["paid_sample"] or 0
            on_time_count = row["on_time_count"] or 0
            ap_balance = row["ap_balance"] or Decimal("0")
            avg_pay_delta = row["avg_pay_delta"]  # timedelta or None
            avg_days_to_pay = (
                avg_pay_delta.total_seconds() / 86400.0
                if isinstance(avg_pay_delta, timedelta)
                else 0.0
            )

            on_time_rate = (on_time_count / paid_sample * 100) if paid_sample else 0.0
            exception_rate = (
                (exception_count / total_count * 100) if total_count else 0.0
            )

            # Amount-weighted on-time rate companion (§1).
            on_time_amount = row["on_time_amount"] or Decimal("0")
            paid_eligible_amount = row["paid_eligible_amount"] or Decimal("0")
            on_time_rate_by_amount = (
                float(on_time_amount) / float(paid_eligible_amount) * 100
                if paid_eligible_amount > 0
                else 0.0
            )
            ...
            scorecard.append(
                {
                    "supplier_id": supplier_id,
                    "supplier": supplier_names.get(supplier_id, ""),
                    ...
                    "avg_dpo": round(avg_days_to_pay, 1),  # Deprecated alias
                    "on_time_rate": round(on_time_rate, 1),
                    "on_time_rate_by_amount": round(on_time_rate_by_amount, 1),
                    "exception_rate": round(exception_rate, 1),
    ...
    def get_supplier_payment_detail(self, supplier_id):
        ...
        invoices = Invoice.objects.filter(
            organization=self.organization, supplier=supplier
        ...
        # Exception breakdown
        exceptions = (
            invoices.filter(has_exception=True)
            .values("exception_type")
        ...
        on_time = sum(
            ...
            for inv in paid
            if inv.paid_date
            and inv.paid_date >= inv.invoice_date
        ...
        exception_history = [
            {
                ...
                "exception_type": inv.exception_type,
                "amount": (
                    float(inv.exception_amount)
                    if inv.exception_amount
                    else float(inv.invoice_amount)
                ),
                "resolved": inv.status == "paid",
                "date": inv.invoice_date.isoformat(),
            }
            for inv in recent_invoices
            if inv.has_exception
        ]
    ...
    def get_pr_approval_analysis(self):
        ...
        prs = PurchaseRequisition.objects.filter(
            organization=self.organization, submitted_date__isnull=False
        )
        prs = self._apply_date_filters(prs, "created_date")

        # Approval time distribution.
        # v3.1 Phase 2 (AN-M1): pull date pairs only via values_list rather
        # than full PR ORM objects; the in-memory loop stays because the
        # buckets cleanly express in 4-way Case/When but a values_list pass
        # over two Date columns is already O(rows) without ORM overhead.
        approved_pairs = prs.filter(approval_date__isnull=False).values_list(
            "submitted_date", "approval_date"
        )

        time_buckets = {"<1 day": 0, "1-2 days": 0, "2-5 days": 0, ">5 days": 0}
        approval_days_list = []
        for submitted, approved in approved_pairs:
        ...
        avg_approval = (
            sum(approval_days_list) / len(approval_days_list)
            if approval_days_list
            else 0
        )

        # Pending by age
        # v3.1 Phase 2 (P-M4): timezone-aware day boundary.
    ...
    def get_pr_detail(self, pr_id):
        ...
        try:
            pr = (
                ...
                .get(id=pr_id, organization=self.organization)
            )
        ...
        return {
            ...
            "rejection_date": (
                pr.rejection_date.isoformat() if pr.rejection_date else None
            ),
            "rejection_reason": pr.rejection_reason,
            "supplier_suggested": (
                {"id": pr.supplier_suggested.id, "name": pr.supplier_suggested.name}
                if pr.supplier_suggested
                else None
            ),
            "purchase_order": (
                {"id": first_po.id, "po_number": first_po.po_number}
                if first_po
                else None
            ),
        }
    ...
    def get_po_by_supplier(self, limit=20):
        ...
        by_supplier = (
            pos.values("supplier__id", "supplier__name")
            .annotate(
                po_count=Count("id"),
                total_value=Sum("total_amount"),
                contract_backed=Count(
                    Case(
                ...
                amended=Count(
                    Case(
                        When(amendment_count__gt=0, then=1), output_field=IntegerField()
                    )
                ),
        ...
        for item in by_supplier:
            ...
            contract_coverage = (
                (contract_backed / po_count * 100) if po_count > 0 else 0
            )
            ...
            result.append(
                {
                    ...
                    "on_contract_pct": round(
                        contract_coverage, 1
                    ),  # Added for frontend compatibility
                    "on_time_rate": on_time_rate,
                    "on_time_eligible_count": eligible,
                    "amendment_rate": (
                        round(item["amended"] / po_count * 100, 1)
                        if po_count > 0
                        else 0
                    ),
                }
    ...
    def get_po_detail(self, po_id):
        ...
        try:
            po = (
                ...
                .prefetch_related("goods_receipts", "invoices")
        ...
        return {
            ...
            "supplier": (
                {"id": po.supplier_id, "name": po.supplier.name}
                if po.supplier
                else None
            ),
            ...
            "contract": (
                {
                    "id": po.contract_id,
                    "contract_number": (
                        po.contract.contract_number if po.contract else None
                    ),
                }
                if po.contract
                else None
            ...
            "original_amount": (
                float(po.original_amount) if po.original_amount else None
            ),
            "linked_pr": (
                {"id": po.requisition.id, "pr_number": po.requisition.pr_number}
                if po.requisition
                else None
            ),
            "goods_receipts": [
