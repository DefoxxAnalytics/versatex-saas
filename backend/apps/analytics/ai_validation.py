...
class LLMResponseValidator:
    ...
    def supplier_names(self) -> Set[str]:
        ...
        if self._supplier_names is None:
            ...
            self._supplier_names.update(name.lower() for name in self._supplier_names)
    ...
    def category_names(self) -> Set[str]:
        ...
        if self._category_names is None:
            ...
            self._category_names.update(name.lower() for name in self._category_names)
    ...
    def validate(self, response: Dict, source_data: Optional[Dict] = None) -> Dict:
        """
    ...
    def _validate_monetary_values(
        self, response: Dict, source_data: Dict
    ) -> List[Dict]:
    ...
    def _validate_suppliers(self, response: Dict) -> List[Dict]:
        ...
        for supplier in mentioned_suppliers:
            if (
                supplier.lower() not in self.supplier_names
                and supplier not in self.supplier_names
            ):
                errors.append(
                    {
                        "field": "suppliers",
    ...
    def _validate_categories(self, response: Dict) -> List[Dict]:
        ...
        for category in mentioned_categories:
            if (
                category.lower() not in self.category_names
                and category not in self.category_names
            ):
                errors.append(
                    {
                        "field": "categories",
    ...
    def _validate_savings_estimates(
        ...
        total_predicted = sum(
            float(i.get("estimated_savings", 0) or 0) for i in insights
        )
    ...
    def _extract_entity_names(self, response: Dict, entity_type: str) -> Set[str]:
    ...
    def _calculate_adjusted_confidence(
        self, original: float, critical: int, error: int, warning: int
    ) -> float:
...
def validate_llm_response(
    organization_id: int, response: Dict, source_data: Optional[Dict] = None
) -> Dict:
    """
