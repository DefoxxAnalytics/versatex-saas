...
class ComplianceService:
    ...
    def get_compliance_overview(self):
        ...
        total_spend = float(
            self.transactions.aggregate(Sum("amount"))["amount__sum"] or 0
        )
        ...
        severity_counts = dict(
            self.violations.values("severity")
        ...
        # v3.1 Phase 2 (AN-M5): use .count() instead of len() — `len()` on a
        # queryset materialises every transaction_id row into a Python list
        # just to take its length. .count() is a single SELECT COUNT(*).
        # Subquery (not list materialisation) is also used for the spend
        # aggregation below so the violating-id set is fetched zero times
        # in Python.
        violating_transaction_ids = self.violations.values_list(
            "transaction_id", flat=True
        ).distinct()
        violating_count = self.violations.values("transaction_id").distinct().count()
        compliant_transactions = total_transactions - violating_count
        compliance_rate = (
            (compliant_transactions / total_transactions * 100)
            if total_transactions > 0
            else 100
        )

        violating_spend = float(
            self.transactions.filter(id__in=violating_transaction_ids).aggregate(
                Sum("amount")
            )["amount__sum"]
            or 0
        )
        compliant_spend = total_spend - violating_spend
        compliance_rate_by_amount = (
            (compliant_spend / total_spend * 100) if total_spend > 0 else 100
        )
        ...
        contracted_suppliers = self.contracts.filter(status="active").values_list(
            "supplier_id", flat=True
        )

        on_contract_spend = float(
            self.transactions.filter(supplier_id__in=contracted_suppliers).aggregate(
                Sum("amount")
            )["amount__sum"]
            or 0
        )
        ...
        maverick_percentage = (
            (maverick_spend / total_spend * 100) if total_spend > 0 else 0
        )
    ...
    def get_maverick_spend_analysis(self):
        ...
        # v3.1 Phase 2 (AN-H3): single grouped query instead of per-contract
        # `.values_list()` (which defeats prefetch_related's cache by re-
        # querying for each contract). Reverse-traverse the M2M from
        # Category to active contracts and collect the distinct ids.
        ...
        contracted_categories = set(
            Category.objects.filter(contracts__in=active_contracts).values_list(
                "id", flat=True
            )
        )
        ...
        # Analyze by category
        category_spend = (
            self.transactions.values("category_id", "category__name")
            .annotate(total=Sum("amount"), count=Count("id"))
            .order_by("-total")
        )
        ...
        return {
            ...
            "maverick_percentage": round(
                (total_maverick_spend / total_spend * 100) if total_spend > 0 else 0, 1
            ),
    ...
    def _generate_maverick_recommendations(
        self, maverick_suppliers, maverick_categories, total_maverick_spend
    ):
            ...
            recommendations.append(
                {
                    ...
                    "potential_savings": sum(
                        s["spend"] * 0.1 for s in high_spend_suppliers
                    ),  # 10% savings estimate
                    "affected_suppliers": [
                        s["supplier_name"] for s in high_spend_suppliers[:5]
                    ],
                    "priority": "high",
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "potential_savings": sum(
                        c["spend"] * 0.05 for c in maverick_categories
                    ),  # 5% savings estimate
                    "affected_categories": [
                        c["category_name"] for c in maverick_categories[:5]
                    ],
                    "priority": "medium",
                }
            )
            ...
            recommendations.append(
                {
                    ...
                    "potential_savings": total_maverick_spend
                    * 0.08,  # 8% savings estimate
                    "priority": "high",
                }
            )
    ...
    def get_policy_violations(self, resolved=None, severity=None, limit=100):
        ...
        violations = self.violations.select_related(
            "transaction", "transaction__supplier", "transaction__category", "policy"
        )
    ...
    def get_violation_trends(self, months=12):
        ...
        monthly_violations = (
            self.violations.filter(created_at__date__gte=cutoff_date)
    ...
    def evaluate_transaction(self, transaction_id):
        ...
        """
        try:
            transaction = self.transactions.select_related("supplier", "category").get(
                id=transaction_id
            )
        ...
        for policy in self.policies:
                ...
                violations.append(
                    {
                        ...
                        "severity": (
                            "high"
                            if float(transaction.amount) > max_amount * 2
                            else "medium"
                        ),
                        "details": {
                            "limit": max_amount,
            ...
            if (
                preferred_suppliers
                and str(transaction.supplier.uuid) not in preferred_suppliers
            ):
                violations.append(
                    {
                        "policy_id": policy.id,
    ...
    def get_supplier_compliance_scores(self):
        ...
        for supplier in suppliers:
            ...
            supplier_violations = self.violations.filter(transaction__supplier=supplier)
            ...
            violation_rate = (
                (violation_count / transaction_count) if transaction_count > 0 else 0
            )
            ...
            scores.append(
                {
                    ...
                    "total_spend": float(
                        supplier_transactions.aggregate(Sum("amount"))["amount__sum"]
                        or 0
                    ),
                    "risk_level": (
    ...
    def _summarize_rules(self, rules):
        ...
        if "required_approval_threshold" in rules:
            summary.append(
                f"Approval required above: ${rules['required_approval_threshold']:,.0f}"
            )
