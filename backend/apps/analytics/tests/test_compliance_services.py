...
from apps.procurement.models import Contract, PolicyViolation, SpendingPolicy
...
class TestComplianceOverview:
    ...
    def test_overview_maverick_spend(
        ...
        off_contract_supplier = SupplierFactory(
            organization=organization, name="Off-Contract"
        )
        for i in range(5):
            TransactionFactory(
                organization=organization,
...
class TestMaverickSpendAnalysis:
    ...
    def test_maverick_analysis_with_contracts(
        ...
        # Off-contract spend
        maverick_supplier = SupplierFactory(
            organization=organization, name="Maverick Supplier"
        )
        for i in range(3):
            TransactionFactory(
                organization=organization,
    ...
    def test_maverick_recommendations(
        ...
        maverick = SupplierFactory(
            organization=organization, name="High Spend Maverick"
        )
        for i in range(10):
            TransactionFactory(
                organization=organization,
...
class TestEvaluateTransaction:
    ...
    def test_evaluate_amount_exceeded(
        ...
        policy_factory(
            name="Amount Limit Policy", rules={"max_transaction_amount": 5000}
        )
        ...
        amount_violations = [
            v for v in violations if v["violation_type"] == "amount_exceeded"
        ]
    ...
    def test_evaluate_no_contract(
        ...
        policy_factory(
            name="Contract Required Policy", rules={"require_contract": True}
        )
        ...
        contract_violations = [
            v for v in violations if v["violation_type"] == "no_contract"
        ]
...
class TestResolveViolation:
    ...
    def test_resolve_violation(
        ...
        result = service.resolve_violation(
            violation.id, admin_user, "Approved by manager"
        )
...
class TestPoliciesList:
    ...
    def test_get_policies(self, organization, policy_factory):
        ...
        policy_factory(name="Test Policy 1", rules={"max_transaction_amount": 10000})
        policy_factory(name="Test Policy 2", rules={"require_contract": True})
...
class TestOrganizationScoping:
    ...
    def test_violations_scoped_to_organization(
        ...
        other_policy = SpendingPolicy.objects.create(
            organization=other_organization, name="Other Policy", rules={}
        )
    ...
    def test_policies_scoped_to_organization(
        ...
        SpendingPolicy.objects.create(
            organization=other_organization, name="Other Org Policy", rules={}
        )
