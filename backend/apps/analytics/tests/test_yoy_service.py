...
class TestYoyDetailedAccuracy:
    ...
    def test_new_category_has_is_new_flag(self, organization, admin_user):
        ...
        result = AnalyticsService(organization).get_detailed_year_over_year(
            use_fiscal_year=True
        )
        new_cat = next(
            c for c in result["category_comparison"] if c["category"] == "NewCat"
        )
        assert new_cat["is_new"] is True
        assert new_cat["is_discontinued"] is False
        legacy = next(
            c for c in result["category_comparison"] if c["category"] == "Legacy"
        )
        assert legacy["is_new"] is False

    def test_single_year_flagged_as_insufficient(
    ...
    def test_calendar_year_uses_calendar_month_labels(self, organization, admin_user):
        self._seed_two_years(organization, admin_user)
        fy_result = AnalyticsService(organization).get_detailed_year_over_year(
            use_fiscal_year=True
        )
        cy_result = AnalyticsService(organization).get_detailed_year_over_year(
