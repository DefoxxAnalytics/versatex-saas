...
class TestAIInsightsServiceInitialization:
    ...
    def test_initialization_with_external_ai(self, organization):
        ...
        service = AIInsightsService(
            organization, use_external_ai=True, ai_provider="openai", api_key="test-key"
        )
...
class TestCostOptimizationInsights:
    ...
    def test_cost_optimization_with_price_variance(self, organization, admin_user):
        ...
        supplier_cheap = SupplierFactory(
            organization=organization, name="Cheap Supplier"
        )
        supplier_expensive = SupplierFactory(
            organization=organization, name="Expensive Supplier"
        )
...
class TestSupplierRiskInsights:
    ...
    def test_supplier_concentration_high(self, organization, category, admin_user):
        ...
        dominant_supplier = SupplierFactory(
            organization=organization, name="Dominant Supplier"
        )
        small_supplier = SupplierFactory(
    ...
    def test_supplier_concentration_critical(self, organization, category, admin_user):
        ...
        dominant_supplier = SupplierFactory(
            organization=organization, name="Mega Supplier"
        )
        other_supplier = SupplierFactory(
            organization=organization, name="Other Supplier"
        )
    ...
    def test_no_concentration_risk(self, organization, category, admin_user):
        ...
        for i in range(5):
            supplier = SupplierFactory(
                organization=organization, name=f"Equal Supplier {i}"
            )
            TransactionFactory(
                organization=organization,
                supplier=supplier,
...
class TestAnomalyInsights:
    ...
    def test_anomaly_detection_high_outliers(
...
class TestConsolidationInsights:
    ...
    def test_consolidation_multiple_suppliers(self, organization, admin_user):
        ...
        category = CategoryFactory(
            organization=organization, name="Consolidation Category"
        )

        # Create 5 suppliers in same category and subcategory for apples-to-apples comparison
        for i in range(5):
            supplier = SupplierFactory(
                organization=organization, name=f"Consol Supplier {i}"
            )
            TransactionFactory(
                organization=organization,
                supplier=supplier,
    ...
    def test_no_consolidation_few_suppliers(self, organization, admin_user):
        ...
        for i in range(2):
            supplier = SupplierFactory(
                organization=organization, name=f"Small Cat Supplier {i}"
            )
            TransactionFactory(
                organization=organization,
                supplier=supplier,
...
class TestGetAllInsights:
    ...
    def test_all_insights_combined(self, organization, admin_user):
        ...
        for i in range(4):
            supplier = SupplierFactory(
                organization=organization, name=f"Mixed Supplier {i}"
            )
        ...
        if len(result["insights"]) > 1:
            ...
            for i in range(len(result["insights"]) - 1):
                current_severity = severity_order.get(
                    result["insights"][i]["severity"], 4
                )
                next_severity = severity_order.get(
                    result["insights"][i + 1]["severity"], 4
                )
                # Allow equal or increasing severity order
                assert current_severity <= next_severity or result["insights"][i].get(
                    "potential_savings", 0
                ) >= result["insights"][i + 1].get("potential_savings", 0)
...
class TestOrganizationScoping:
    ...
    def test_insights_scoped_to_organization(
        ...
        category2 = CategoryFactory(
            organization=other_organization, name="Other Category"
        )
        supplier2 = SupplierFactory(
            organization=other_organization, name="Other Supplier"
        )
        for i in range(5):
            TransactionFactory(
...
class TestDeduplicateSavings:
    ...
    def test_all_insights_includes_deduplication_summary(
        ...
        for i in range(5):
            supplier = SupplierFactory(
                organization=organization, name=f"Dedup Supplier {i}"
            )
            TransactionFactory(
                organization=organization,
                supplier=supplier,
