...
def supplier_with_history(organization, supplier, category, admin_user):
    ...
    for m in range(24):
        TransactionFactory(
            ...
            uploaded_by=admin_user, amount=Decimal('25000'),
    ...
    for m in range(3):
        TransactionFactory(
            organization=organization, supplier=supplier, category=category,
            uploaded_by=admin_user, amount=Decimal('20000'),
...
class TestContractsListUtilization:
    ...
    def test_utilization_excludes_pre_contract_history(self, organization, supplier_with_history):
...
class TestContractCoverageWindow:
    ...
    def test_coverage_scoped_to_trailing_year(self, organization, category, admin_user):
        ...
        for m in range(24):
            TransactionFactory(
                organization=organization, supplier=old_supplier, category=category,
                uploaded_by=admin_user, amount=Decimal('200000'),
        ...
        contracted_supplier = SupplierFactory(organization=organization, name='ContractedSupplier')
        for m in range(6):
            TransactionFactory(
                organization=organization, supplier=contracted_supplier, category=category,
                uploaded_by=admin_user, amount=Decimal('20000'),
                date=today - timedelta(days=30 * m + 5),
...
class TestContractPerformanceMonthlyAverage:
    ...
    def test_sporadic_spend_does_not_inflate_monthly_average(self, organization, supplier, category, admin_user):
        today = date.today()
        contract = Contract.objects.create(
            organization=organization,
            supplier=supplier,
        ...
        for m in [0, 4, 8]:
            TransactionFactory(
                organization=organization, supplier=supplier, category=category,
                uploaded_by=admin_user, amount=Decimal('10000'),
                date=today - timedelta(days=30 * m + 15),
                invoice_number=f'SPOR-{m}',
            )

        perf = ContractAnalyticsService(organization).get_contract_performance(contract.id)
