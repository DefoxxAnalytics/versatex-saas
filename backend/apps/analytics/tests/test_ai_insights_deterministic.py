...
def risk_fixture(organization, admin_user):
    ...
    for i in range(10):
        TransactionFactory(
            organization=organization, supplier=dominant, category=cat,
            uploaded_by=admin_user, amount=Decimal('70000'),
    ...
    for i in range(5):
        TransactionFactory(
            organization=organization, supplier=small_1, category=cat,
    ...
    for i in range(3):
        TransactionFactory(
            organization=organization, supplier=small_2, category=cat,
            uploaded_by=admin_user, amount=Decimal('5000'),
            subcategory='sub_alpha',
    ...
    for i in range(2):
        TransactionFactory(
            organization=organization, supplier=small_3, category=cat,
            uploaded_by=admin_user, amount=Decimal('3000'),
            subcategory='sub_alpha',
...
def price_variance_fixture(organization, admin_user):
    ...
    for i in range(8):
        TransactionFactory(
            organization=organization, supplier=cheap, category=cat,
    ...
    for i in range(8):
        TransactionFactory(
            organization=organization, supplier=expensive, category=cat,
            uploaded_by=admin_user, amount=Decimal('1800'),  # 80% more
            subcategory='sub_one',
...
def _assert_insight_shape(insights):
    ...
    required = {'id', 'type', 'severity', 'confidence', 'title', 'description',
