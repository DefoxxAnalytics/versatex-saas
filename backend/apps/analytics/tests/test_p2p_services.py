...
class TestAvgDaysToPayHelper:
    ...
    def test_rejects_paid_before_issued_as_data_quality(self):
        invoices = [
            ...
            self._inv(date(2024, 2, 1), date(2024, 1, 20)),   # paid before issued → dropped
...
class TestDaysToPayResponseKeys:
    ...
    def test_supplier_payments_overview_exposes_both_aliases(self, paid_invoice_fixture):
        service = P2PAnalyticsService(paid_invoice_fixture)
    ...
    def test_supplier_payments_scorecard_exposes_both_aliases(self, paid_invoice_fixture):
        service = P2PAnalyticsService(paid_invoice_fixture)
        ...
        assert row['days_to_pay'] == row['avg_days_to_pay'] == row['dpo'] == row['avg_dpo']

    def test_supplier_payment_detail_exposes_both_aliases(self, paid_invoice_fixture, supplier):
        service = P2PAnalyticsService(paid_invoice_fixture)
...
class TestCluster6Relabels:
    ...
    def _seed_match_fixture(self, organization, supplier):
        from apps.procurement.models import PurchaseOrder, Invoice
    ...
    def test_matching_overview_exposes_amount_weighted_rates(self, organization, supplier):
        ...
        assert result['exceptions']['percentage_by_amount'] == result['exception_rate_by_amount']
...
class TestComplianceAmountWeightedRate:
    ...
    def test_compliance_overview_exposes_both_rates(self, organization, supplier, category, admin_user):
        from apps.procurement.tests.factories import TransactionFactory
        from apps.procurement.models import SpendingPolicy, PolicyViolation
        from datetime import date
        ...
        big_clean = TransactionFactory(
            ...
            uploaded_by=admin_user, amount=Decimal('100000'),
            date=date.today(), invoice_number='CLEAN-1',
        )
        for i in range(10):
            tx = TransactionFactory(
                organization=organization, supplier=supplier, category=category,
                uploaded_by=admin_user, amount=Decimal('100'),
                date=date.today(), invoice_number=f'BAD-{i}',
            )
...
class TestPredictiveGrowthWindows:
    ...
    def _run(self, values):
        ...
        metrics = {}
        if len(values) >= 24:
            cur = sum(values[-12:]); prev = sum(values[-24:-12])
            metrics['yoy_growth'] = round((cur - prev) / prev * 100, 2) if prev > 0 else 0
        if len(values) >= 12:
            cur = sum(values[-6:]); prev = sum(values[-12:-6])
            metrics['six_month_growth'] = round((cur - prev) / prev * 100, 2) if prev > 0 else 0
        if len(values) >= 6:
            cur = sum(values[-3:]); prev = sum(values[-6:-3])
            metrics['three_month_growth'] = round((cur - prev) / prev * 100, 2) if prev > 0 else 0
        return metrics
