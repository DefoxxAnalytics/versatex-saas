...
0 3 * * * cd /home/deploy/versatex-saas && set -a && . ./.env && set +a && ./scripts/backup-postgres.sh >> /var/log/pg-backup.log 2>&1
```
...
VOLUME_PATH=$(docker volume inspect -f '{{ .Mountpoint }}' versatex-saas_media_volume)
...
```cron
30 3 * * * /home/deploy/versatex-saas/scripts/backup-media.sh >> /var/log/media-backup.log 2>&1
```
